...
// Film-pressure coupling requires the exchange to be enabled AND the
// film_pressure_coupling master flag set. The flag defaults to true, and the
// parser overrides an explicit false back to true, so this is effectively
// governed by whether the exchange itself is enabled.
...
// C, m^T C m = 9K with m = identity2, so K = identity2 . (C identity2) / 9.
// This is exactly d sigma'_m / d eps_v = -dp_conf/deps_v, the MECHANICAL
// stiffness the film-pressure coupling needs for the p-u tangent (mu_lR film
// delta, exchange equation) AND its one-Psi transpose, the swelling-stress
// u-eps tangent
// (+(1-phi_M)*b*K). Using ONE formula keeps the two sides of the Maxwell pair
// on the SAME K. NOTE: this is the MECHANICAL drained K only; it
// is no longer used as a swelling-stress modulus (the K_sw eigenstress form is
// retired
...
inline double getPotentialPressureTolerance(
    std::optional<PotentialExchangeParameters> const&
        potential_exchange_parameters)
{
    return getPotentialPressureTolerance(potential_exchange_parameters
                                             ? &*potential_exchange_parameters
                                             : nullptr);
}
...
inline PotentialExchangeUpdateData computePotentialExchangeUpdate(
    ...
    double const rho_lR_exchange_input =
    ...
    double const mu_lR_vdw = 0.0, double const dmu_lR_vdw_drho_lR = 0.0,
    ...
    double const fd_jacobian_perturbation = 1e-8)
{
    ...
    // rho_LR depends on liquid pressure in RM through beta_LR = (1/rho)
    // drho/dp.
    ...
    {
        auto const compute_rho_L_hat =
            [&](double const p_L_ip_eval, double const rho_LR_eval)
        {
            auto const macro_potential_eval = computeYoungLaplaceMacroPotential(
                p_L_ip_eval, rho_LR_eval, pressure_tolerance);
            double const alpha_M_effective_eval = alpha_bar * rho_LR_eval / mu;
            double const mu_LR_active_eval =
                use_macro_potential_for_active_exchange
                    ? macro_potential_eval.mu_LR
                    : p_L_ip_eval / rho_LR_eval;
            double const rho_lR_eval = use_custom_micro_density_for_exchange
                                           ? rho_lR_exchange
                                           : rho_LR_eval;
            double const mu_lR_active_eval =
                use_vdw_micro_potential_for_active_exchange
        ...
        {
            OGS_FATAL(
                "computePotentialExchangeUpdate requires finite h > 0 for FD "
                "Jacobian, got {:g} (from fd_jacobian_perturbation={:g}, "
                "p_L_ip={:g}).",
    ...
    double const dmu_lR_exchange_input_dpL =
        use_vdw_micro_potential_for_active_exchange
            ? (use_custom_dmu_lR_vdw_dpL ? dmu_lR_vdw_dpL
...
inline double microPotentialSignFactorFromParameters(
    PotentialExchangeParameters const& potential_exchange_params)
{
    return microPotentialSignFactor(
...
struct PotentialExchangeLocalSolveContext
{
    ...
    // tension-positive). Film-pressure coupling only. NaN
    // sentinel = "stress not supplied" -> the film term self-disables, so a
    // default-constructed context (e.g. the eigenstress-difference driver,
    // which has no stress in scope) gets NO film term. Set explicitly at the
    // call sites that have EffectiveStressData available.
    ...
    // do not set it explicitly). Set at the assembly / micro-solve context
    // sites from the evaluated MPL biot_coefficient.
    ...
    // -> the partner's quadratic (p_conf-chain) piece is dropped, leaving the
    // linear
    // -(Pi + n_l*Pi')*eps_v/rho_lR conjugate (the GP eigenstress-difference
    // driver and tests that have no stiffness in scope get this reduced, finite
    // form). Set at the assembly / micro-solve context sites from the elastic
    // stiffness.
    ...
    double const n_l_floor)
{
    ...
    {
        double const phi_prev_sum =
            std::clamp(std::max(0.0, local_context.phi_M_prev) +
        ...
        double const delta_eps_v = local_context.volumetric_strain -
    ...
    MacroPorosityUpdateMode const macro_porosity_update_mode)
{
    constexpr double porosity_upper = 1.0 - 1e-12;
    double const phi_prev_sum =
        std::clamp(std::max(0.0, phi_M_prev) + std::max(0.0, phi_m_prev), 0.0,
                   porosity_upper);
    ...
    double const phi_M_prev_safe =
        std::min(std::max(0.0, phi_M_prev), phi_safe);
    double const phi_m_prev_safe = std::min(
        std::max(0.0, phi_m_prev), std::max(0.0, phi_safe - phi_M_prev_safe));
        ...
        MacroPorosityUpdateMode::ReferenceAdditiveRate)
    {
        ...
        std::call_once(
            ...
            []
            {
                INFO(
                    "DSM: "
                    "macro_porosity_update_mode='additive_macro_porosity_rate_"
                    "mode' now evaluates the hierarchical porosity split.");
            });
    }
    ...
    double const phi_m = std::clamp((1.0 - phi_M) * n_l_safe, 0.0,
                                    std::max(0.0, phi_safe - phi_M));
...
// PHYSICS FIX: The returned quantity is the aggregate SOLID
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
    ...
    // PHYSICS FIX: Previously returned 1 - phi_M_prev. Now
    // returns the previous aggregate solid fraction 1 - n_l_prev to match
    // the corrected active definition.
    double const n_l_prev_safe =
        std::clamp(std::max(0.0, n_l_prev), 0.0, 1.0 - 1e-12);
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
    ...
    double const rho_l0 = std::max(
        1e-16, potential_exchange_params.micro_liquid_density_reference);
    double const a_rho =
    ...
    for (int iter = 0; iter < max_iterations; ++iter)
    {
        ...
        if (std::abs(residual) <= tolerance * std::max(1.0, std::abs(rho_lR)))
        {
            converged = true;
            break;
        }
    ...
    if (!converged)
    {
        ...
        std::call_once(
            ...
            {
                WARN(
                    "DSM: reduced microscale liquid-density EOS did not "
                    "converge at least once; using the last Newton iterate.");
            });
    }
    ...
    (void)rhs;
    double const common =
        (rho_lR - rho_LR) * a_rho * b_rho * std::pow(omega_l, b_rho - 1.0);
    double const dg_drho = 1.0 + common * (n_l_safe / denominator);
    // dg/dn_l with nS FROZEN: common * domega/dn_l|_{nS} =
    // common*(rho_lR/denom).
    double dg_dn = common * (rho_lR / denominator);
    // F2 (tangent-only): under current_porosity_split nS = 1 - n_l
    // is LIVE, so the EOS slaved drho_lR/dn_l picks up the domega/dnS*(dnS/dnl)
    // chain. domega/dnS = -omega/nS, dnS/dnl = -1 -> chain = +omega/nS, added
    // to domega/dn_l, i.e. dg_dn += common*(omega_l/nS). Reference mode: nS
    // constant
    ...
    double const drho_lR_dnl =
        (std::isfinite(dg_drho) && std::abs(dg_drho) > 1e-20) ? -dg_dn / dg_drho
                                                              : 0.0;
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
    ...
    return computeReducedMicroLiquidDensity(n_l, rho_LR, active_nS,
...
// confining_pressure_p_conf sentinel, so flag OFF or stress-not-supplied
// (default-constructed context, e.g. the eigenstress-difference driver) leaves
// `out` BIT-FOR-BIT unchanged.
//
...
// WITHOUT a hard clamp. Gate g = t*(2 - t), t = (n_l_cap - n_l)/w in [0,1]: C1
// at onset (t=1) but a NONZERO slope at the cutoff edge (t->0, dg/dt->2), so d
// mu_lR/d n_l stays nonzero where the saturated equilibrium sits -> the
// pressure-block diagonal stays conditioned (a smoothstep, dg/dt=0 at t=0,
// would re-singularise it). floor == 0 -> unchanged (bit-for-bit).
    ...
    PotentialExchangeParameters const& params)
{
    ...
    {
        g_cut =
            t * (2.0 - t);  // C1 at t=1; dg/dt=2 at t=0 (nonzero edge slope)
        dg_dt = 2.0 * (1.0 - t);
    }
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
        ...
        potential_exchange_params.film_energy_route == FilmEnergyRoute::Exact)
    {
        ...
        double const kappa_ex = potential_exchange_params.film_strain_kappa ==
        ...
        auto const pair = computeStrainedFilmEnergyPair(
            ...
            // parse scalar. H2 fix: use the
            // SAME effective K the bare out.mu_lR was built with, so
            // g_cut = out.mu_lR/pair.mu_bare_pre stays the macro-floor cutoff
            // factor under live K (scalar-mode bit-for-bit; matches
            // :768/:2084).
        ...
        // L1 fix: pair.dmu_bare_dnl_pre was computed with frozen
        // nS (dnS_dnl = 0), but out.dmu_lR_dnl carries the caller's F2
        // chain (dnS_dnl = -1 under CurrentPorositySplit; see computeActive-
        // MicroPotential). Mixing the two legs corrupts dg_dnl under
        ...
        FilmStrainCouplingMode::Off)
    {
        ...
        double const sign_sf =
            microPotentialSignFactorFromParameters(potential_exchange_params);
        // Live K(rho_d): rho_d = rho_SR*(1-phi) from the
        ...
        auto const film_state = computeStrainedFilmState(
            ...
            potential_exchange_params.micro_solid_density_reference,
            potential_exchange_params.hamaker_constant,
            potential_exchange_params.specific_surface, sign_sf, K_aug_sf,
            potential_exchange_params.potential_augmentation_exponent,
            potential_exchange_params.micro_water_content_floor,
        ...
        strained.d2mu_lR_dnl2 *= film_state.dw_eff_dnl * film_state.dw_eff_dnl;
    ...
    // K_drained from the context (mechanical drained bulk modulus). NaN
    // sentinel (default-constructed context / GP eigenstress-difference driver)
    // -> drop the 0.5*b*K_drained*eps_v^2 quadratic so the partner reduces to
    // the finite linear conjugate -(Pi + n_l*Pi')*eps_v/rho_lR.
    double const K_drained = std::isfinite(local_context.drained_bulk_modulus)
...
inline ReducedMicroLiquidDensityData computePreviousMicroLiquidDensity(
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
    double const previous_nS = computePreviousMicroSolidVolumeFraction(
        n_l_prev, local_context, potential_exchange_params);
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
    ...
    double const volumetric_strain_rate =
        dt_safe > 0.0 ? (local_context.volumetric_strain -
                         local_context.volumetric_strain_prev) /
                            dt_safe
                      : 0.0;
    ...
    auto evaluate = [&](double const n_l)
    {
        ...
        auto micro_potential = computeVanDerWaalsMicroPotential(
            ...
            0.0 /*dnS_dnl*/,
            potential_exchange_params.micro_water_content_floor);
        // Increment E: fold the film delta into mu_lR (and dmu_lR_dnl, which
        // the analytic predictor Jacobian below reads through micro_potential)
        // using the SAME micro density the vdW formula consumed. No-op when the
        // flag is OFF / p_conf NaN -> predictor is bit-for-bit the bare-vdW
        // path.
        applyFilmPressureMicroPotential(
            micro_potential, n_l, micro_liquid_density.rho_lR, active_nS,
    ...
    for (int iter = 0; iter < max_iterations; ++iter)
    {
        ...
        {
            delta_n_l *= 0.5;
            n_l_candidate = std::clamp(n_l + delta_n_l, n_l_floor, n_l_ceiling);
            auto const [retry_residual, retry_micro_potential, retry_exchange,
                        retry_micro_density] = evaluate(n_l_candidate);
            ...
            increment_tolerance * std::max(1.0, std::abs(n_l)))
        {
            ...
            auto const [final_residual, final_micro_potential, final_exchange,
                        final_micro_density] = evaluate(n_l_candidate);
            (void)final_residual;
...
// Localized floating-point-contraction guard.
...
// numerics. Portable STDC pragma (file scope; gcc honours this) +
// clang-specific statement-scope `#pragma clang fp contract(off)` inside the
// body (clang honours that form most reliably).
...
#pragma clang fp contract(off)
    ...
    auto evaluate = [&](double const n_l, double const rho_lR)
    {
        ...
        auto micro_potential = computeVanDerWaalsMicroPotential(
            ...
            0.0 /*dnS_dnl*/,
            potential_exchange_params.micro_water_content_floor);
        // Increment E: fold the film delta into mu_lR with the in-iteration
        // micro density rho_lR (the 2x2 unknown the vdW formula just consumed).
        // The 2x2 local Jacobian is finite-difference over THIS lambda, so it
        // picks up the film term in both residuals automatically. No-op when
        // the flag is OFF / p_conf NaN -> bit-for-bit the bare-vdW path.
        ...
        auto const exchange = computePotentialDrivenMassExchange(
            alpha_M_effective, mu_LR_active, mu_lR_active);
        // REV-scale liquid apparent density: phi_m * rho_lR (hierarchical
        // split). phi_m = (1-phi)/(1-n_l)*n_l. Previously n_l*rho_lR (aggregate
        // scale).
        double const phi_cs =
            std::isfinite(local_context.phi)
                ? std::clamp(local_context.phi, 0.0, 1.0 - 1e-12)
    ...
    // through the shared helper chain) are reused; the ONLY piece `evaluate`
    // does not carry is the live-nS chain in d mu_lR/d n_l (it passes
    // dnS_dnl=0), so the analytic evaluator re-runs the vdW helper with the
    // correct dnS_dnl and re-applies the film coupling, reproducing the TOTAL
    // n_l-derivative the FD of `evaluate` sees.
    //
    // Closed forms (phi fixed in the local solve; one_minus_n_l = max(1e-12,
    // 1-n_l) matching `evaluate`):
    //   rho_l            = (1-phi)/(1-n_l) * n_l * rho_lR [kg/m^3] d rho_l/d
    //   rho_lR = (1-phi)/(1-n_l) * n_l        (= rho_l/rho_lR) [-] d rho_l/d
    //   n_l    = (1-phi) * rho_lR / (1-n_l)^2                  [kg/m^3]
    //   d(mass_res)/d n_l    = d rho_l/d n_l*(1 - dt*eps_dot)
    //                          + dt*alpha_M*d mu_lR/d n_l   (rho_l_hat =
    //                          alpha_M*(mu_LR-mu_lR), d rho_l_hat/d n_l =
    //                          -alpha_M*d mu_lR/d n_l; sign flips with the
    //                          -dt*).
    //   d(mass_res)/d rho_lR = d rho_l/d rho_lR*(1 - dt*eps_dot)
    //                          + dt*alpha_M*d mu_lR/d rho_lR
    //   d(dens_res)/d n_l    = -d rho_lR_EOS/d n_l   (EOS drho_lR_dnl; carries
    //   the
    //                          live-nS chain in CurrentPorositySplit, 0
    //                          otherwise)
    //   d(dens_res)/d rho_lR = 1   (rho_lR_EOS depends on n_l & rho_LR only,
    //   not
    ...
    auto evaluate_analytic_jacobian = [&](double const n_l, double const rho_lR)
    {
        ...
        auto micro_potential = computeVanDerWaalsMicroPotential(
            ...
            potential_exchange_params.specific_surface,
            microPotentialSignFactorFromParameters(potential_exchange_params),
            // MERGE FIX: use the SAME live K(rho_d) the residual `evaluate`
            // uses. This lambda originated on a branch that predated the
            // repo-wide conversion of every computeVanDerWaalsMicroPotential
            // call site to effectiveAugmentationPrefactor; a later merge left
            // this one call site on the parse-time scalar:
            // with potential_augmentation_prefactor_live_dry_density=true the
            // analytic 2x2 would then be the derivative of a DIFFERENT
            // potential than the residual. That is tangent-only, but do NOT
            ...
            // every MS33 suite PRJ.
            effectiveAugmentationPrefactor(potential_exchange_params,
                                           local_context.phi),  // K [J/kg]
            potential_exchange_params.potential_augmentation_exponent, dnS_dnl,
            potential_exchange_params.micro_water_content_floor);
        // Re-apply the film coupling so d mu_lR/d n_l and d mu_lR/d rho_lR pick
        // up the integrable Maxwell partner exactly as `evaluate` did.
        applyFilmPressureMicroPotential(micro_potential, n_l, rho_lR, active_nS,
                                        local_context,
                                        potential_exchange_params);
        double const dmu_lR_dnl = micro_potential.dmu_lR_dnl;  // J/kg per n_l
        double const dmu_lR_drho_lR =
            micro_potential.dmu_lR_drho_lR;  // (J/kg)/(kg/m^3)
        ...
        double const J12 =
            ...
            dt_safe * alpha_M_effective *
                dmu_lR_drho_lR;  // [-] (kg/m^3 per kg/m^3)
    ...
    // The analytic micro 2x2 local Jacobian is now the
    // DEFAULT. It was verified correct earlier: analytic == FD central
    ...
    // micro-tangent error). REQUIREMENT: this path
    // needs <scaling>false</scaling> in the Eigen <linear_solver> block of the
    // DSM PRJs. With Eigen's IterScaling (Ruiz) enabled, the row/column
    // equilibration overflows on the intrinsically near-singular pressure block
    // (cond ~5.8e22 from micro_liquid_density_a= 1e-16 + Bishop saturation
    // cutoff + tiny intrinsic-k); with scaling off and SparseLU the system
    // factorizes cleanly. The fp-contract(off) pragma and the u-side analytic
    // blocks (still OFF) are left as-is. Deliberately decoupled from
    // use_fd_jacobian_for_exchange (the block #3 macro p-p tangent flag), which
    // keeps its own default so block #3 stays analytic exactly as parent;
    // setting use_fd_jacobian_for_exchange still forces the FD micro path here.
    //
    // DEFAULT REVERTED true -> false.
    ...
    // so they no longer reproduce the previously approved reference VTUs:
    ...
    // With this flag false the tree reproduces all 6 references
    // step-for-step identically to the pre-change binary (VERIFIED 6/6, 66/66
    // field comparisons). The "land it as default" decision
    // was therefore parked, NOT withdrawn: re-enabling is this one line, and
    // requires re-baselining the Model III + VII reference VTUs first.
    // RE-ENABLED, with the references re-baselined in the same
    // change. The parking rationale above is the historical record of that
    // decision.
    // Measured for this tree/toolchain before re-enabling: unit suite
    // 1418/1418; dd1400/dd1600/ dd1800/ModelIV reproduce their committed
    // references 11/11 fields each under the analytic path (step paths
    // unchanged: 308/311/308/637); Model III (405 -> 376 steps, 2 rejected ->
    // 0) and Model VII (682 -> 675) change step path only, and their references
    // are re-baselined in this change. The new III/VII baselines are
    // run-to-run AND cross-build bit-identical (exact 0.0 between two
    // independent builds; the file-scope FP_CONTRACT OFF pragma is what pins
    // this). u-side blocks remain OFF (unchanged, known-unsafe). NOTE the
    // earlier "analytic == FD to round-off" claim above was measured against
    // an earlier residual; the residual has since changed (live
    // K(rho_d), strained film), so it is NOT re-verified for this tree.
    constexpr bool use_analytic_micro_jacobian = true;

    for (int iter = 0; iter < max_iterations; ++iter)
    {
        auto const [mass_residual, density_residual, micro_potential,
                    exchange] = evaluate(n_l, rho_lR);
        ...
        // Default: the analytic micro 2x2, selected by the
        // use_analytic_micro_jacobian constexpr above. The finite-difference
        // path is taken only when that constexpr is turned off, or when the
        // use_fd_jacobian_for_exchange project-file flag forces it.
        if (potential_exchange_params.use_fd_jacobian_for_exchange ||
            !use_analytic_micro_jacobian)
        {
            // Finite-difference path: central difference of the full
            // `evaluate` over (n_l, rho_lR).
        ...
        double const delta_n =
            (-mass_residual * J22 + density_residual * J12) / det;
        double const delta_rho =
            (J21 * mass_residual - J11 * density_residual) / det;
    ...
    MicroMacroMassStorageCoupledSolveData const& coupled_update)
{
    ...
    auto const compatibility_output = computeCompatibilityMicroHydraulicOutput(
        coupled_update.n_l, rho_LR, local_context, potential_exchange_params);
    ...
    *micro_porosity = transport_porosity_update.phi_m;
    ...
    auto& porosity =
        std::get<ProcessLib::ThermoRichardsMechanics::PorosityData>(
            ...
            .phi;
    porosity =
        transport_porosity_update.phi_M + transport_porosity_update.phi_m;
    variables.porosity = porosity;
    variables_prev.porosity = transport_porosity_update.phi_M_prev +
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
    ...
    double const rho_lR_effective =
        potential_exchange_params.local_nonlinear_solve_mode ==
            ...
            ? computeReducedMicroLiquidDensity(n_l, rho_lR, active_nS,
                                               potential_exchange_params)
                  .rho_lR
            : rho_lR;
    ...
    auto out = computeVanDerWaalsMicroPotential(
        n_l, rho_lR_effective, active_nS,
        potential_exchange_params.micro_solid_density_reference,
        potential_exchange_params.hamaker_constant,
        potential_exchange_params.specific_surface,
        microPotentialSignFactorFromParameters(potential_exchange_params),
    ...
    // p_L_m writer computeCompatibilityMicroHydraulicOutput, the
    // residual/Jacobian assembly) AND stays identical to the mass-storage 2x2
    // solve (increment E). Flag OFF or NaN p_conf -> the helper is a no-op
    // (pure vdW), bit-for-bit. Uses the SAME density (rho_lR_effective) the vdW
    // formula above consumed.
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
    ...
    double const n_l_ref = std::max(
        1e-16,
        potential_exchange_params.initial_micro_water_content.value_or(
            potential_exchange_params.micro_solid_volume_fraction_reference));
    ...
    // use_micro_liquid_density_for_micro_pressure should be true in all PRJ
    // files.
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
    ...
    auto eval_at = [&](double const n_l)
    {
        ...
        auto const micro_liquid_density =
            use_mass_storage
                ? std::optional<
                      ReducedMicroLiquidDensityData>{computeActiveMicroLiquidDensity(
                      n_l, rho_LR, local_context, potential_exchange_params)}
                : std::nullopt;
    ...
    auto const prev_micro_liquid_density =
        use_mass_storage
            ? std::optional<
                  ReducedMicroLiquidDensityData>{computePreviousMicroLiquidDensity(
                  n_l_prev, rho_LR, local_context, potential_exchange_params)}
            : std::nullopt;
    ...
    if (use_mass_storage)
    {
        auto const coupled_update = solveReferenceMassStorageCoupledState(
            ...
            prev_micro_liquid_density ? prev_micro_liquid_density->rho_lR
                                      : rho_LR,
            dt_safe, rho_LR, alpha_bar, mu, macro_potential, local_context,
            potential_exchange_params);
    ...
    for (int iter = 0; iter < max_iterations; ++iter)
    {
        ...
        if (use_mass_storage)
        {
            ...
            residual = rho_l - rho_l_prev - dt_safe * exchange.rho_l_hat;
            residual -= dt_safe * rho_l * volumetric_strain_rate;

            jacobian =
                micro_liquid_density->drho_l_dn_l - dt_safe * drho_l_hat_dn_l;
            jacobian -= dt_safe * micro_liquid_density->drho_l_dn_l *
                        volumetric_strain_rate;
        }
        else
        {
            residual = n_l - n_l_prev - dt_safe * exchange.rho_l_hat / rho_LR;
            ...
            increment_tolerance * std::max(1.0, std::abs(n_l)))
        {
            auto const [micro_potential_candidate, exchange_candidate,
                        micro_density_candidate] = eval_at(n_l_candidate);
    ...
    if (!converged)
    {
        ...
        if (use_mass_storage)
        {
            ...
            explicit_increment = dt_safe * exchange_prev.rho_l_hat /
                                 std::max(1e-16, micro_density_prev->rho_lR);
            explicit_increment += dt_safe * rho_l_prev_fallback *
                                  volumetric_strain_rate /
                                  std::max(1e-16, micro_density_prev->rho_lR);
        }
        else
        {
            explicit_increment = dt_safe * exchange_prev.rho_l_hat / rho_LR;
            if (use_microstate_storage_mode)
            {
                explicit_increment +=
        ...
        out.n_l =
            std::clamp(n_l_prev + explicit_increment, n_l_floor, n_l_ceiling);
        auto const [micro_potential_fallback, exchange_fallback,
                    micro_density_fallback] = eval_at(out.n_l);
        ...
        std::call_once(once,
                       ...
                       {
                           WARN(
                               "DSM: local implicit n_l solve did not converge "
                               "at least once; falling back to explicit n_l "
    ...
    double const rho_lR_micro = std::numeric_limits<double>::quiet_NaN())
{
    ...
    // residual with rho_lR eliminated along r2=0 (1x1 reduction; the EOS
    // already returns the slaved drho_lR/dn_l). TANGENT-ONLY: the converged
    // forward solve is untouched; only the Newton Jacobian is made consistent.
    // The converged (n_l, rho_lR) are threaded in by the caller;
    // micro_potential / exchange are the values evaluated at that converged
    // state.
    if (potential_exchange_params.local_nonlinear_solve_mode ==
        LocalNonlinearSolveMode::ScalarReferenceMassStorage)
    {
        double const eps_v_rate = (local_context.volumetric_strain -
                                   local_context.volumetric_strain_prev) /
                                  dt_safe;
        double const time_factor = 1.0 - dt_safe * eps_v_rate;

        // Converged n_l (fall back to n_l_prev only if the caller omitted it).
        double const n_l = std::max(
            1e-16, std::isfinite(n_l_converged) ? n_l_converged : n_l_prev);
        ...
        double const rho_lR =
            (std::isfinite(rho_lR_micro) && rho_lR_micro > 0.0) ? rho_lR_micro
                                                                : eos.rho_lR;
        ...
        // p_L channel at fixed n_l. rho_lR varies with p_L only through the
        // bulk rho_LR appearing additively in the EOS: g = rho_lR - rho_LR -
        // rho_l0*exp(-a*omega^b) = 0 (omega = n_l*rho_lR/(nS*rho_SR), no
        // rho_LR), so drho_lR/drho_LR|_{fixed n_l} = -dg/drho_LR / dg/drho_lR =
        // 1/dg_drho.
        ...
        if (drho_LR_dpL != 0.0)
        {
            double const rho_SR = std::max(
                1e-16, potential_exchange_params.micro_solid_density_reference);
            double const a_rho = std::max(
                1e-16, potential_exchange_params.micro_liquid_density_a);
            double const b_rho = std::max(
                1e-16, potential_exchange_params.micro_liquid_density_b);
            double const denom = std::max(1e-16, nS) * rho_SR;
            double const common =
                (rho_lR - rho_LR) * a_rho * b_rho *
                std::pow(std::max(1e-16, eos.omega_l), b_rho - 1.0);
        ...
        // displacement via phi(eps_v). The converged local n_l therefore
        // carries an implicit strain channel
        ...
        // the Jacobian side; see L3 below.) PREDICTED (not yet
    ...
    double const dmu_first_dpL_fixed_n =
        ...
        macro_potential.dmu_LR_drho_LR * drho_LR_dpL;
    double const dmu_second_dpL_fixed_n =
...
// eigenstress (L3), wired at the M2 displacement-Jacobian site.
//
// RESIDUAL-SAFE: reads ONLY the already-converged (n_l, rho_lR,
// micro_potential, exchange) the caller threads in -- it never re-solves, never
// mutates the forward state, and is identically 0 outside
// ScalarReferenceMassStorage (and when dt<=0). dr_dn_l is rebuilt here by the
// SAME expression as computeImplicitNlDpL so numerator and denominator are
// linearized about one state. Returns 0 on a singular/non-finite dr_dn_l
// (matching the dn_l/dpL guard), so a degenerate tangent silently drops rather
// than poisoning K[u,u].
inline double computeImplicitNlDK(
    double const n_l_prev, double const dt, double const rho_LR,
    double const mu, VanDerWaalsMicroPotentialData const& micro_potential,
    PotentialDrivenMassExchangeData const& exchange,
    PotentialExchangeLocalSolveContext const& local_context,
    PotentialExchangeParameters const& potential_exchange_params,
    double const n_l_converged = std::numeric_limits<double>::quiet_NaN(),
    double const rho_lR_micro = std::numeric_limits<double>::quiet_NaN())
{
    ...
    // dr/dK: K enters r ONLY via rho_l_hat = exchange(mu_lR(.;K)); the mass
    // term rho_l = c*f*rho_lR has no K dependence (the EOS omega has no K). So
    //   dr/dK = -dt * drho_l_hat/dmu_lR * dmu_lR/dK.            [kg/m^3 per
    //   (J/kg)]
...
inline void updateMicroscaleHydraulicState(
    StatefulData<DisplacementDim>& state_current,
    StatefulDataPrev<DisplacementDim> const& state_previous,
    double const p_cap_ip, double const rho_LR, double const mu,
    double const dt, MPL::VariableArray& variables,
    ...
    PotentialExchangeParameters const* const potential_exchange_parameters)
{
    ...
    *n_l = n_l_prev_value;
        ...
        LocalNonlinearSolveMode::ScalarReferenceMassStorage)
    {
        ...
        // REV-scale previous liquid apparent density: phi_m_prev * rho_lR_prev.
        // local_context.phi_m_prev = (1-phi_M_prev)*n_l_prev (hierarchical
        // split).
        ...
        applyReferenceMassStorageLocalState<DisplacementDim>(
            state_current, state_previous, variables, variables_prev, rho_LR,
            local_context, potential_exchange_params, coupled_update);
        return;
    }
    ...
    *n_l = n_l_update.n_l;
    // Keep dsm_micromacro-mode rho_lR evolution consistent with the
    // dsm_micromacro bridge: rho_lR is updated from the active reduced micro
    // EOS.
    *rho_lR =
        computeActiveMicroLiquidDensity(n_l_update.n_l, rho_LR, local_context,
                                        potential_exchange_params)
            .rho_lR;
    ...
    PotentialExchangeParameters const* const potential_exchange_parameters)
{
    ...
    auto const phi_M_prev = std::get<PrevState<
        ProcessLib::ThermoRichardsMechanics::TransportPorosityData>>(
                                ...
                                ->phi;
    auto const n_l =
        std::max(1e-16, *std::get<MicroWaterContent>(state_current));
...
inline void updateTotalPorosityState(
    StatefulData<DisplacementDim>& state_current,
    StatefulDataPrev<DisplacementDim> const& state_previous, double& phi,
    MPL::VariableArray& variables, MPL::VariableArray& variables_prev,
    PotentialExchangeParameters const* const potential_exchange_parameters)
{
    if (!isPotentialExchangeEnabled(potential_exchange_parameters))
    ...
    if (potential_exchange_parameters->local_nonlinear_solve_mode ==
        ...
        potential_exchange_parameters->local_nonlinear_solve_mode ==
            LocalNonlinearSolveMode::ScalarReferenceMassStorage)
    {
        // In scalar dsm_micromacro-storage mode, micro porosity is
        // support-state only. Keep the process porosity state on the medium-law
        // carrier.
        return;
    }
...
computeReferenceMicroPorositySwellingStressIncrement(
    double const n_l_prev, double const n_l, double const n_S,
    double const rho_lR, double const rho_lR_prev, double const rho_LR,
    ...
    // TOTAL porosity phi for live K(rho_d); NaN sentinel
    ...
    double const total_porosity = std::numeric_limits<double>::quiet_NaN())
{
    ...
    // residual (the operational-branch residual defect, |W|/scale=0.93, that
    // the exact route exists to cure is NOT cured if the eigenstress half
    // stays operational).
    // The pair gives the drained-line LEVEL sigma_sw_m = -nS*n_l*(Pi(w_eff) +
    // b*K_d*eps); telescope to the step increment as the operational branch
    // does:
    //   delta_sigma_sw = (sigma_sw_m_curr - sigma_sw_m_prev)*I.
    // PHYSICS TRADEOFF (predicted, not yet verified): the pair's eigenstress
    // is on the DRAINED LINE p_conf = -K_d*eps_v, whereas the operational
    // branch uses the
    // ACTUAL GP p_conf (held fixed across the step). On the drained line the
    // two agree; off it (e.g. fully confined, eps_v~0 with p_conf growing) they
    // differ by the off-line p_conf excursion. This is the deliberate one-Psi
    // consistency choice. Exact route requires Kinematic coupling (create-time
    // validated, mirrored at the mu fold above).
    if (potential_exchange_params.film_pressure_coupling &&
        potential_exchange_params.film_strain_coupling !=
            FilmStrainCouplingMode::Off &&
        potential_exchange_params.film_energy_route == FilmEnergyRoute::Exact &&
    ...
    // routed a pressure through an elastic strain (the bulk modulus K_sw does
    // NOT belong in a residual stress); the chosen form is
    //   sigma_sw(n) = -phi_m * p_film,
    //   phi_m       = (1 - phi_M)*n_l = n_S*n_l   (MICRO porosity),
    //   p_film      = Pi(n_l) - b*p_conf,
    // tension-positive. Here Pi is the BARE van-der-Waals disjoining pressure
    // at n_l (BEFORE the film delta),
    //   Pi(n_l) = -rho_lR_used * mu_lR_vdw(n_l)   (FULL p^disj, density
    //   mirrored to the hydraulic p_L_m choice exactly as the OFF branch
    //   below),
    ...
    //   n_l*p_film_curr)*identity2.
    // p_conf_prev is NOT threaded here (only the current p_conf is), so p_conf
    // is HELD FIXED across the step (current p_conf used for both p_film
    // terms): the p_conf->stress coupling then enters only through the outer
    // (Newton) iteration as p_conf updates step to step. With the n_l weighting
    // the -b*p_conf term does NOT cancel (the weights n_l_prev, n_l differ):
    ...
    // K_sw is GONE from this residual entirely. This makes sigma_sw
    // compressive, ~0 at dry (n_l->0), growing in magnitude with wetting, and
    // proportional to Pi (density-correct).
    if (potential_exchange_params.film_pressure_coupling)
    {
        // The bare-vdW disjoining law needs physical vdW parameters (mirrors
        // the OFF-branch up-front check so the message is
        // swelling-law-specific).
        ...
        // Bare van-der-Waals micro potential at n_l and n_l_prev, computed
        // EXACTLY as the OFF branch does (same active_nS, sign factor and
        // augmentation args) -> the BARE Pi, BEFORE any film delta.
        double const active_nS_prev_film =
            computeActiveMicroSolidVolumeFraction(
                n_l_prev, PotentialExchangeLocalSolveContext{}, params);
        double const active_nS_curr_film =
            computeActiveMicroSolidVolumeFraction(
        ...
        // Density mirrors the hydraulic p_L_m choice EXACTLY (see OFF branch
        // and computeCompatibilityMicroHydraulicOutput): confined micro-liquid
        ...
        // > Pi_prev as it wets => -(Pi_curr - Pi_prev) < 0 increment, but
        // starting from a non-zero dry offset the running sum drifted tensile).
        ...
        // BOTH p_film_prev and p_film_curr): p_conf_prev is not threaded here,
        // and this is exactly the prior code's intent (the p_conf->stress
        // coupling then enters only through the outer Newton iteration as
        // p_conf updates step to step). Unlike the old (unweighted) form the
        // -b*p_conf term does NOT cancel here, because the n_l weights differ:
        ...
        // n_S passed in is the REV macro-solid fraction (1 - phi_M); phi_m =
        // n_S*n_l.
        delta_sigma_sw.noalias() +=
    ...
    double const mu_lR_curr =
        computeVanDerWaalsMicroPotential(
            n_l, rho_lR, active_nS_curr, params.micro_solid_density_reference,
            params.hamaker_constant, params.specific_surface, sign_factor,
            K_aug_sw /*live K(rho_d), J/kg*/,
            params.potential_augmentation_exponent, 0.0 /*dnS_dnl*/,
            params.micro_water_content_floor)
            .mu_lR;
...
computeSwellingStressIncrement(
    ...
    double const total_porosity = std::numeric_limits<double>::quiet_NaN())
{
    return computeReferenceMicroPorositySwellingStressIncrement<
        DisplacementDim>(n_l_prev, n_l, n_S, rho_lR, rho_lR_prev, rho_LR, C_el,
                         potential_exchange_params, biot_coefficient, p_conf,
                         eps_v, eps_v_prev, total_porosity);
}

template <int DisplacementDim>
inline void updateSwellingState(
    MaterialPropertyLib::Phase const& solid_phase, double const rho_LR,
    MathLib::KelvinVector::KelvinMatrixType<DisplacementDim> const& C_el,
    StatefulData<DisplacementDim>& state_current,
    StatefulDataPrev<DisplacementDim> const& state_previous,
    MPL::VariableArray& variables, MPL::VariableArray& variables_prev,
    ...
    double const biot_coefficient = 1.0)
{
    ...
    // Film-pressure coupling: confining pressure
    // p_conf = -tr(sigma_eff)/3 (>0 in compression, OGS tension-positive) from
    // the CURRENT effective stress, threaded into the swelling stress p_film
    // term. Only when the flag is ON; otherwise the NaN sentinel keeps the term
    // out and the OFF (disjoining-eigenstress) branch is bit-for-bit unchanged.
    // Mirrors the p_conf_micro_solve computed at the production call sites.
    ...
    sigma_sw.sigma_sw += computeSwellingStressIncrement<DisplacementDim>(
    ...
    for (unsigned ip = 0; ip < n_integration_points; ip++)
    {
...

        {
            ...
            if (isPotentialExchangeEnabled(potential_exchange_params_ptr))
            {
                ...
                auto const transport_porosity =
                    std::get<ProcessLib::ThermoRichardsMechanics::
                        ...
                        this->current_states_[ip])
                        .phi;
                double const porosity_safe =
                    std::clamp(std::max(0.0, porosity), 0.0, 1.0 - 1e-12);
                ...
                n_l_initial = std::clamp(
                    (porosity_safe - transport_porosity_safe) / one_minus_phi_M,
                    1e-12, porosity_safe);
            ...
            if (isPotentialExchangeEnabled(potential_exchange_params_ptr))
            {
                ...
                double const transport_safe_for_split =
                    std::clamp(std::max(0.0, transport_porosity_init_for_split),
                               0.0, porosity_safe_for_split);
                phi_m_initial =
                    std::clamp((1.0 - transport_safe_for_split) * n_l_initial,
                               1e-16, porosity_safe_for_split);
            }
            *n_l = n_l_initial;
            ...
            **phi_m_prev = phi_m_initial;
            ...
            if (isPotentialExchangeEnabled(potential_exchange_params_ptr))
            {
                ...
                auto& S_L_m =
                    std::get<MicroSaturation>(this->current_states_[ip]);
                auto& S_L_m_prev = std::get<PrevState<MicroSaturation>>(
                    this->prev_states_[ip]);
                *p_L_m = compatibility_output.p_L_m;
                ...
                **S_L_m_prev = compatibility_output.S_L_m;
                ...
                auto& transport_porosity =
                    std::get<ProcessLib::ThermoRichardsMechanics::
                        ...
                        this->current_states_[ip])
                        .phi;
                auto& transport_porosity_prev =
                    std::get<PrevState<ProcessLib::ThermoRichardsMechanics::
                                           TransportPorosityData>>(
                        this->prev_states_[ip]);
                auto const transport_porosity_update =
                    computeTransportPorosityUpdate(
                        porosity, transport_porosity, phi_m_initial,
                        n_l_initial,
                        /*volumetric_strain=*/0.0,
                        /*volumetric_strain_prev=*/0.0,
                        potential_exchange_params_ptr
                               ...
                               std::vector<double>& local_rhs_data)
{
    ...
    for (unsigned ip = 0; ip < n_integration_points; ip++)
    {
            ...
            this->process_data_.micro_porosity_parameters)
        {
            ...
            double const pressure_tolerance =
                getPotentialPressureTolerance(potential_exchange_params_ptr);
            ...
            if (potential_exchange_enabled)
            {
                auto const n_l = std::max(
                    1e-16,
                    *std::get<MicroWaterContent>(this->current_states_[ip]));
                auto const transport_porosity_prev =
                    std::get<PrevState<ProcessLib::ThermoRichardsMechanics::
                                           TransportPorosityData>>(
                        this->prev_states_[ip])
                        ->phi;
                auto const phi_m_prev = **std::get<PrevState<MicroPorosity>>(
                    this->prev_states_[ip]);
                // Confining pressure p_conf = -tr(sigma_eff)/3 (>0 in
                // compression). Threaded into the context ONLY when
                // film-pressure coupling is ON, so computeActiveMicroPotential
                // folds the SAME mu_lR(p_film) used by the n_l solve. Flag OFF
                // -> NaN -> no film term -> bit-for-bit identical to the
                // pre-film code.
                ...
                {
                    auto const rho_lR_state = *std::get<MicroLiquidDensity>(
                        this->current_states_[ip]);
                ...
                //
                // Film-pressure coupling: when the flag is ON,
                ...
                {
                    double const Pi_mc =
                        p_L_m;  // Pa, disjoining = -rho*mu_lR > 0
                    // The INTEGRABLE partner takes eps_v + K_drained (not
                    // p_conf) and carries NO explicit (1-phi_M) factor (the n_S
                    // referencing cancels in the specific potential --
                    // equipresence note in PotentialExchange.h), so p_conf /
                    // n_S / one-minus-n_l are no longer formed here; mirrors
                    // the L717 micro-solve fold.
                    double const rho_mc =
                        (std::isfinite(rho_lR_exchange_input) &&
                         rho_lR_exchange_input > 0.0)
                            ? rho_lR_exchange_input
                            : rho_LR;
                    // dPi/dn_l = (Pi/mu_lR)*dmu_lR_dnl (Pi=-rho*mu_lR;
                    // density-agnostic)
                    ...
                    // Pi''/Pi = mu_lR''/mu_lR, matching the dPi_dnl_mc
                    // convention.
                    double const d2Pi_dnl2_mc =
                        (std::abs(mu_lR_vdw) > 1e-300)
                            ? (Pi_mc / mu_lR_vdw) * micro_potential.d2mu_lR_dnl2
                            : 0.0;
                    // INTEGRABLE Maxwell mechanical partner (Option B):
                    // the SINGLE mu_lR_mech form, UNGATED (no Pi
                    // Heaviside) -- mirrors the micro-solve fold so the
                    // macro-exchange mu_lR equals the micro-solve mu_lR
                    // (equipresence). PARAMETER-FREE: Pi/Pi'/Pi'' from the vdW
                    // potential, eps_v + b (=alpha) from the assembly,
                    // K_drained from the elastic stiffness C_el evaluated in
                    // this ip loop.
            ...
            auto const potential_exchange_result =
                computePotentialExchangeUpdate(
            ...
            rhs.template segment<pressure_size>(pressure_index).noalias() +=
                N_p.transpose() * potential_exchange_result.exchange.rho_L_hat *
                w;
        }
    ...
    {
        auto pressure_mass_block_diag =
            M.template block<pressure_size, pressure_size>(pressure_index,
                                                           pressure_index);
        pressure_mass_block_diag =
            pressure_mass_block_diag.colwise().sum().eval().asDiagonal();
    }
}
            ...
            material_state_data)
{
    ...
    auto& S_L = std::get<ProcessLib::ThermoRichardsMechanics::SaturationData>(
                    state_current)
                    .S_L;
    auto const S_L_prev =
        std::get<
    ...
    auto const alpha =
        medium->property(MPL::PropertyType::biot_coefficient)
            .template value<double>(variables, x_position, t, dt);
    *std::get<ProcessLib::ThermoRichardsMechanics::BiotData>(
        constitutive_data) = alpha;
    ...
    variables.grain_compressibility = beta_SR;
    std::get<ProcessLib::ThermoRichardsMechanics::SolidCompressibilityData>(
        constitutive_data)
        .beta_SR = beta_SR;
    ...
    *std::get<LiquidDensity>(constitutive_data) = rho_LR;
    ...
    double const DeltaS_L_Deltap_cap =
        (p_cap_ip == p_cap_prev_ip)
            ? dS_L_dp_cap
            : (S_L - S_L_prev) / (p_cap_ip - p_cap_prev_ip);
    std::get<SaturationSecantDerivative>(constitutive_data)
        .DeltaS_L_Deltap_cap = DeltaS_L_Deltap_cap;
    ...
    *std::get<ProcessLib::ThermoRichardsMechanics::LiquidViscosityData>(
...

    {
        ...
        auto const p_L_m_prev =
            std::get<PrevState<MicroPressure>>(state_previous);
        auto& S_L_m = std::get<MicroSaturation>(state_current);
        auto const S_L_m_prev =
            std::get<PrevState<MicroSaturation>>(state_previous);
    ...
    // Only under film coupling; NaN sentinel otherwise -> the partner is inert
    // and the flag-off micro solve is bit-for-bit unchanged.
    double const K_drained_micro_solve =
        isFilmPressureCouplingEnabled(potential_exchange_parameters)
            ? drainedBulkModulusFromStiffness<DisplacementDim>(C_el)
            : std::numeric_limits<double>::quiet_NaN();
    updateMicroscaleHydraulicState<DisplacementDim>(
        state_current, state_previous, p_cap_ip, rho_LR, mu, dt, variables,
        variables_prev,
        {.phi = phi,
    ...
    updatePorositySplitState<DisplacementDim>(state_current, state_previous,
                                              phi, variables, variables_prev,
                                              potential_exchange_parameters);
    updateTotalPorosityState<DisplacementDim>(state_current, state_previous,
                                              phi, variables, variables_prev,
                                              potential_exchange_parameters);
    std::get<ProcessLib::ThermoRichardsMechanics::PorosityData>(
        constitutive_data)
        .phi = std::get<ProcessLib::ThermoRichardsMechanics::PorosityData>(
                   state_current)
                   .phi;
    updateSwellingState<DisplacementDim>(
        solid_phase, rho_LR, C_el, state_current, state_previous, variables,
        variables_prev, x_position, t, dt, potential_exchange_parameters,
        /*biot_coefficient=*/alpha);

    // Enforce phi_m <= phi_total and phi_M = phi_total - phi_m >= 0 in the
    // constitutive state. This clamp belongs to the potential-exchange
    // (double-structure) path only: it reads the MicroPorosity state, which is
    // updated solely on that path. On the legacy <micro_porosity> path the
    // state stays at its initial value, so gating on the presence of
    // <micro_porosity> alone would overwrite the macro porosity that
    // computeMicroPorosity() has just computed. The parser rejects
    // potential_exchange.enabled without <micro_porosity>, so this condition
    // implies micro_porosity_parameters.has_value() for any loadable setup.
    if (isPotentialExchangeEnabled(potential_exchange_parameters))
    {
        auto& phi_M_cs =
            std::get<
        ...
        variables_prev.transport_porosity = std::get<PrevState<
            ProcessLib::ThermoRichardsMechanics::TransportPorosityData>>(
                                                state_previous)
                                                ->phi;
    }
    ...
    if (medium->hasProperty(MPL::PropertyType::transport_porosity))
    {
        ...
        // Pi-path and legacy-DSM modes: phi_M already set by
        // updatePorositySplitState / updateSwellingStressAndVolumetricStrain;
        // no property evaluation needed.
    }
    else
    {
        // No transport_porosity medium property. In Pi-path / DSM mode
        // variables.transport_porosity is already phi_M from
        // updatePorositySplitState. Only fall back to total porosity for plain
        // RM (no micro-porosity split).
        if (!isPotentialExchangeEnabled(potential_exchange_parameters) &&
...

    {
        ...
        *std::get<StiffnessTensor<DisplacementDim>>(constitutive_data) =
            std::move(C);
    }
                         ...
                         std::vector<double>& local_Jac_data)
{
    ...
    for (unsigned ip = 0; ip < n_integration_points; ip++)
    {
        ...
        std::get<StrainData<DisplacementDim>>(state_current).eps.noalias() =
            B * u;
        ...
        assembleWithJacobianEvalConstitutiveSetting(
            ...
            this->process_data_.micro_porosity_parameters,
            this->getPotentialExchangeParameters(), this->solid_material_,
            this->material_states_[ip]);
            ...
            this->process_data_.micro_porosity_parameters)
        {
            ...
            if (potential_exchange_enabled)
            {
                ...
                auto const phi_m_prev = **std::get<PrevState<MicroPorosity>>(
                    this->prev_states_[ip]);
                // Film-pressure coupling: thread p_conf into
                // the context so computeActiveMicroPotential folds the SAME
                ...
                //
                // Film-pressure coupling: when ON, mu_lR
                // ALREADY carries the film term (folded in
                // computeActiveMicroPotential), so SKIP this strain-view block
                // to avoid double-counting; the film p-u tangent is added in
                // the dedicated ON branch below. Flag OFF -> this block runs
                // unchanged (bit-for-bit Jacobian).
                if (!film_pressure_coupling)
                {
                    double const Pi_mc =
                        p_L_m;  // Pa, disjoining = -rho*mu_lR > 0
                    ...
                    double const d2Pi_dnl2_mc =
                        (std::abs(mu_lR_vdw) > 1e-300)
                            ? (Pi_mc / mu_lR_vdw) * micro_potential.d2mu_lR_dnl2
                            : 0.0;
                    // Drained bulk modulus from the elastic tangent stiffness,
                    // the SAME mechanical K the L717 fold uses (here
                    // reconstructed via computeElasticTangentStiffness,
                    // mirroring the ON p-u block below). dp_conf/deps_v =
                    // -dsigma'_m/deps_v.
                    double const K_drained_mc =
                        drainedBulkModulusFromStiffness<DisplacementDim>(
                            ip_data_[ip].computeElasticTangentStiffness(
                    ...
                    //   d rho_L_hat/d u = alpha_M * (dmu_lR_mech/deps_v) * m^T
                    //   B,
                    // m = identity2 (eps_v = m^T B u). The integrable partner
                    // is SMOOTH in eps_v (NO Heaviside gate), so this fires
                    // whenever the exchange is active (no gate_open guard).
                    // Without this block the tangent is inconsistent and Newton
                    // diverges (Task-13 Pi blow-up). Sign mirrors the K[p,p]
                    // exchange (-=).
                ...
                if (film_pressure_coupling && mu > 0.0)
                {
                    ...
                    // tangent is built from the SAME disjoining law. active_nS
                    // is context-independent (Reference -> ref fraction;
                    // CurrentPorositySplit -> 1 - n_l). Thread dnS_dnl exactly
                    // as computeActiveMicroPotential does (so Pi' here equals
                    // the residual's Pi' including the F2 live-nS chain).
                    ...
                    double const dPi_dnl_pu =
                        -rho_film * vdw_pu.dmu_lR_dnl;  // Pa/n_l
                    double const d2Pi_dnl2_pu =
                        -rho_film * vdw_pu.d2mu_lR_dnl2;  // Pa/n_l^2
                    // Drained bulk modulus = dp_conf/deps_v, the SAME
                    // mechanical K threaded into the residual fold.
                    // (Reconstruct C_el as the enable_dsm_swelling_up_jacobian
                    // block does below.)
                    ...
                    //     route != Exact): mu_lR = bare(w_eff(eps_v))
                    //     + b*p_conf(eps_v)/rho, so
                    //       dmu_lR/deps_v = bare'(w_eff)*dw_eff/deps_v
                    //                       + b*(dp_conf/deps_v)/rho,
                    //     with bare'(w_eff) = the law's d/d(arg) (its
                    //     dmu_lR_dnl BEFORE the dw_eff_dnl chain) and
                    //     dp_conf/deps_v = -K_drained
                    //     (drainedBulkModulusFromStiffness = dsigma' _m/deps_v
                    //     = -dp_conf/deps_v).
                    //   Exact (film_energy_route == Exact): mu_lR +=
                    //     g_cut*pair.mu_mech with the bare evaluated at the
                    //     TRUE n_l (no eps_v), so dmu_lR/deps_v = g_cut*pair
                    //     .dmu_mech_deps_v.
                        ...
                        FilmStrainCouplingMode::Off)
                    {
                        ...
                        if (pep_m1.film_energy_route == FilmEnergyRoute::Exact)
                        {
                            ...
                            // g_cut = mu_lR(post macro-floor
                            // cutoff)/mu_bare_pre, matching the residual fold.
                            // The residual mu_lR_vdw already carries
                            // the cutoff; recover g via bare_pre. |mu_bare_pre|
                            // > 0 by the bare law FATALs.
                            double const g_cut_m1 =
                                vdw_pu.mu_lR / pair_m1.mu_bare_pre;  // [-]
                            dmu_lR_deps_v_film =
                                g_cut_m1 *
                                pair_m1.dmu_mech_deps_v;  // J/kg/strain
                        }
                        else
                        {
                            ...
                            auto const film_state_m1 = computeStrainedFilmState(
                                ...
                                rho_film, pep_m1.micro_solid_density_reference,
                                pep_m1.hamaker_constant,
                                pep_m1.specific_surface, sign_m1, K_aug_m1,
                                pep_m1.potential_augmentation_exponent,
                                pep_m1.micro_water_content_floor,
                                rho_film /*rho_pi*/);
                            // Bare law at w_eff; its dmu_lR_dnl is
                            // d(bare)/d(arg) (the arg plays the role of n_l),
                            // so multiplying by dw_eff/deps_v gives
                            // d(bare(w_eff))/deps_v.
                            ...
                            dmu_lR_deps_v_film =
                                dbare_deps_v +
                                dmuload_deps_v;  // J/kg per strain
                        }
                    }
                    local_Jac
                        .template block<pressure_size, displacement_size>(
                    ...
                    // SCHEME: the LIVE table is LOG-linear,
                            ...
                            double const w_phi_pu =
                                ...
                                (variables.effective_pore_pressure -
                                 variables_prev.effective_pore_pressure) *
                                    beta_SR;  // [-]
                            // N1 fix: PorosityFromMassBalance
                            // CLAMPS phi to [phi_min, phi_max] (its value(),
                            // PorosityFromMassBalance.cpp). When the clamp
                            // is active phi is flat in eps_v -> dphi/deps_v =
                            // 0; the analytic (alpha-phi)/(1+w) would be a
                            // spurious nonzero tangent. The bounds are private,
                            // so detect the clamp by comparing the unclamped
                            // law value to the stored (clamped) phi: a mismatch
                            // beyond a relative floor means a bound is active.
                ...
                use_fd_jacobian_for_direct_macro_derivative =
                    potential_exchange_params_ptr->use_fd_jacobian_for_exchange;
                ...
                if (!use_fd_jacobian_for_direct_macro_derivative)
                {
                    ...
                    auto const macro_potential =
                        computeYoungLaplaceMacroPotential(p_L_ip, rho_LR,
                                                          pressure_tolerance);
                    double const alpha_M_effective = alpha_bar * rho_LR / mu;
                    auto const exchange = computePotentialDrivenMassExchange(
                    ...
                    // use_micro_liquid_density_for_micro_pressure, i.e. always
                    // in mass-storage mode); NaN -> the helper recomputes via
                    // EOS.
                    double const dn_l_dpL = computeImplicitNlDpL(
                        n_l_prev, p_L_ip, dt, rho_LR, drho_LR_dpL, alpha_bar,
                        mu, macro_potential, micro_potential, exchange,
                    ...
                    // n_l(p_L) chain through the gate
                    // (dmu_lR_film_dnl*dn_l_dpL, since dmu_lR_dnl includes the
                    // gate's dPi_gate/dn_l term) and (ii) the rho_lR(p_L)
                    // channel dmu_lR_film_drho_lR*drho_lR_dpL (here rho_lR ==
                    // bulk rho_LR in ScalarReferenceStorage mode, so the bulk
                    // drho_LR_dpL is the right pairing, matching the vdW
                    // convention above). NOT included (by design): the direct
                    // p_conf(p_L) channel via Bishop effective stress -- that
                    // lagged u-p coupling is left to the consistent-tangent
                    // block (enable_dsm_swelling_up_ jacobian, default OFF) per
                    // the spec.
                    dmu_lR_vdw_dpL =
                    ...
                    // present in the residual but ABSENT from K[u,u]/K[u,p]
                    // (the only swelling strain tangent was the constexpr-false
                    // dead block below, which carries no dK/dphi chain). Wire
                    // ONLY the live-K chain here -- NOT the pre-existing
                    // swelling u-p/u-u term set OFF
                    // (enable_dsm_swelling_up_ jacobian stays at its default;
                    ...
                    // Jacobian bit-for-bit. The residual eigenstress uses live
                    // K in BOTH its film-ON branch and its OFF (disjoining)
                    // branch
                    // (computeReferenceMicroPorositySwellingStressIncrement
                    // L2380/L2389 use K_aug_sw), so this gate is the live-K
                    // flag, NOT film_pressure_coupling -- otherwise the 1b
                    // family (live K, film coupling OFF) keeps the residual
                    // dependence without the matching tangent (the 1b_A step-1
                    // divergence). JACOBIAN-ONLY: residual untouched.
                    if (potential_exchange_params_ptr
                            ->potential_augmentation_prefactor_live_dry_density)
                    {
                        ...
                        if (dK_dphi_sw != 0.0 && is_pfmb)
                        {
                            ...
                            // updateSwellingStateWithMicroPorosity,
                            // L2325-2371).
                            ...
                            double const rho_pi_prev_sw =
                                pep_sw.use_micro_liquid_density_for_micro_pressure
                            ...
                            // strained w_eval is taken ONLY inside the
                            // residual's film_pressure_coupling block (L2073);
                            // the OFF (disjoining) branch uses w_eval = n_l. So
                            // gate the strained state on film_pressure_coupling
                            // here too -- NOT on film_strain_coupling --
                            // otherwise 1b_B (kinematic coupling, film OFF)
                            // would strain w_eval while its residual
                            // eigenstress does not.
                            double w_eval_prev_sw = n_l_prev_sw;
                            double w_eval_curr_sw = n_l;
                            // L3 also needs d(w_eval_curr)/dn_l of the
                            // residual's eval argument: 1 on the OFF branch
                            // (w_eval = n_l), dw_eff/dn_l on the strained
                            // branch.
                            ...
                            // augmentation channel is linear in K ->
                            // dmu_lR_dK).
                            ...
                            //        - n_l*rho_curr*dmu_curr/dK ).  [Pa per
                            //        J/kg]
                            ...
                            double const w_phi_sw =
                                ...
                                (variables.effective_pore_pressure -
                                 variables_prev.effective_pore_pressure) *
                                    beta_SR;  // [-]
                            // N1 clamp guard: zero the chain when
                            // PorosityFromMassBalance clamps phi to a bound
                            // (flat in eps_v/p there); detect via unclamped vs
                            // stored phi (bounds are private). Same logic as
                            // the live-K p-u block below.
                            ...
                            // chain. This is the EXPLICIT-K channel (M2):
                            // K(phi) varies with strain at FIXED converged n_l.
                            ...
                            // K=K(phi(eps_v)), so the converged n_l ALSO moves
                            // with strain through K -- a channel the explicit-K
                            // term above (n_l held) omits and the fixed-n_l
                            // dn_l/dpL does not carry (L3). By the chain
                            // rule on delta_sigma_sw(n_l(K(phi)),K):
                            ...
                            // for ScalarExchange/ReferenceStorage). The
                            // explicit partial d(delta_sigma_sw)/dn_l is taken
                            // at FIXED K and FIXED prev state, from the
                            // residual increment delta_sigma_sw =
                            // n_S*(n_l_prev*Pi_prev
                            //                        - n_l*Pi_curr)
                            //                        (L2406/L2301),
                            ...
                            //              * dmu_lR_curr/dw_eval
                            //              * dw_eval/dn_l ),
                            // where dmu_lR_curr/dw_eval =
                            // vdw_curr_sw.dmu_lR_dnl (the law's derivative
                            // w.r.t. its eval argument) and dw_eval/dn_l = 1 on
                            // the OFF branch (w_eval=n_l) or
                            // film_state.dw_eff_dnl on the strained branch --
                            // EXACTLY the residual's argument-chain
                            // (L2384/L2073). Mirrors the residual's dnS_dnl=0
                            // (active_nS frozen) and held rho_curr.
                            // JACOBIAN-ONLY: the local FORWARD n_l solve and
                            // the residual are untouched; this only completes
                            // the assembled displacement tangent. Folded into
                            // the SAME scalars so it rides the identical
                            // C*C_el^{-1}*identity2 map below. Off / frozen /
                            // clamped edge / non-mass-storage -> the factors
                            // are 0 -> bit-for-bit. Exact route (H1) sources
                            // the residual eigenstress from the one-Psi pair,
                            // NOT this telescoped form, so the telescoped
                            // d(delta_sigma_sw)/dn_l here is not the assembled
                            // residual's partial there. Restrict the implicit
                            // chain to the telescoped-residual regimes (OFF +
                            // film-ON-operational) so it stays a true partial
                            // of the assembled eigenstress and is a bit-for-bit
                            // no-op on the exact-route 1b_B (which is already
                            // converging; its tangent is left untouched).
                    ...
                    // ENABLED: the u-side
                    // swelling tangent completes the consistent Maxwell
                    // Jacobian. The film-ON block below (K[u,p] + K[u,u])
                        ...
                        film_pressure_coupling)
                    {
                        ...
                        //   d sigma_sw/d n_l   = -(1 - phi_M)*( p_film +
                        //   n_l*dPi/dn_l ), d sigma_sw/d eps_v = -(1 -
                        //   phi_M)*n_l*b*(dp_conf/deps_v)
                        //                      = +(1 - phi_M)*n_l*b*K_drained
                        // (dp_conf/deps_v = -K_drained, the MECHANICAL drained
                        // bulk modulus; Pi and n_l do not depend on eps_v at
                        // fixed n_l). K_sw is GONE here -- it no longer routes
                        // a pressure through an elastic strain. Pi is the BARE
                        // vdW disjoining pressure (recomputed below, NOT
                        // micro_potential.mu_lR which already carries the film
                        // delta). Both pieces map to R_u through dsigma'/d(.) =
                        // C * C_el^{-1} * d sigma_sw/d(.).
                        ...
                        double const b_swj =
                            alpha;  // Biot b == poroelastic alpha

                        // BARE vdW Pi(n_l) and dPi/dn_l, density mirrored to
                        // the residual's p_L_m choice (micro rho_lR when
                        // enabled, bulk otherwise) and treated
                        // density-agnostically in dPi/dn_l (matching the LIVE
                        // Maxwell-conjugate block: dPi/dn_l = -rho *
                        // dmu_lR_bare/dn_l).
                        double const active_nS_bare_swj =
                            computeActiveMicroSolidVolumeFraction(
                                n_l, PotentialExchangeLocalSolveContext{},
                                *potential_exchange_params_ptr);
                        auto const vdw_bare_swj =
                            computeVanDerWaalsMicroPotential(
                                ...
                                // Live K(rho_d): rho_d = rho_SR*(1-phi) (total
                                // porosity in scope); off -> parse scalar.
                                // NOTE: the live-K dK/dphi chain
                                // is NOT added in this default-OFF block (dead
                                // code, enable_dsm_swelling_up_jacobian=false);
                                // wire it per the live p-u block above if ever
                                // enabled.
                                effectiveAugmentationPrefactor(
                                    *potential_exchange_params_ptr,
                                    phi),  // K [J/kg]
                        ...
                        double const p_conf_drain_swj =
                            std::isfinite(p_conf_assembly) ? p_conf_assembly
                                                           : 0.0;
                        double const p_film_swj =
                            Pi_bare_swj - b_swj * p_conf_drain_swj;

                        // u-p (via n_l): d sigma_sw/d n_l * dn_l/dpL on
                        // identity2, with d sigma_sw/d n_l = -(1-phi_M)*(
                        // p_film + n_l*dPi/dn_l ).
                        ...
                        MathLib::KelvinVector::KelvinVectorType<
                            DisplacementDim> const dsigma_sw_deps_v =
                    ...
                    else if (enable_dsm_swelling_up_jacobian)
                    {
                        ...
                        // fraction n_S = 1 - phi_M, matching the residual
                        // caller (updateSwellingStateWithMicroPorosity, line
                        // ~1671).
                        double const phi_M_swj =
                            std::get<ProcessLib::ThermoRichardsMechanics::
                                         TransportPorosityData>(
                        ...
                        // only matters for nonlinear tangents.
                        auto const& C_consistent_swj =
                            *std::get<StiffnessTensor<DisplacementDim>>(
                                constitutive_data);
                        auto const C_el_swj =
            ...
            auto const potential_exchange_result =
                computePotentialExchangeUpdate(
                    ...
                    dmu_lR_vdw_dpL, use_fd_jacobian_for_direct_macro_derivative,
                    fd_jacobian_perturbation);
            local_rhs.template segment<pressure_size>(pressure_index)
                .noalias() += N_p.transpose() *
                              potential_exchange_result.exchange.rho_L_hat * w;
    ...
    for (unsigned ip = 0; ip < n_integration_points; ip++)
    {
        ...
        double const K_drained_micro_solve =
            isFilmPressureCouplingEnabled(
                ...
                : std::numeric_limits<double>::quiet_NaN();
        updateMicroscaleHydraulicState<DisplacementDim>(
            this->current_states_[ip], this->prev_states_[ip], p_cap_ip, rho_LR,
            mu, dt, variables, variables_prev,
            {.phi = phi,
             .phi_M_prev = transport_porosity_prev_value,
             .phi_m_prev = phi_m_prev_value,
             .volumetric_strain = variables.volumetric_strain,
        ...
        // Enforce phi_m <= phi_total and phi_M = phi_total - phi_m >= 0. When
        // the micro water content n_l approaches the total porosity phi under
        // confinement, the hierarchical split can produce phi_M < 0. Cap micro
        // porosity at the total and set phi_M = phi - phi_m >= 0. This is the
        // output-field counterpart of the constitutive clamp above and, like
        // it, applies to the potential-exchange path only: the MicroPorosity
        // state it reads is not updated on the legacy <micro_porosity> path.
        ...
        {
            // No transport_porosity medium property. In Pi-path / DSM mode
            // variables.transport_porosity is already phi_M from the Gate fix
            // above. Only fall back to total porosity for plain RM (no
            // micro-porosity split).
