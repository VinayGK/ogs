...
// residual scales.
//
// Sample-state parameter values mirror the unit tests in
// DSMMicroMacroSingleIntegrationPoint.cpp (hamaker 6.0e-20 J, Sa 1000 m^2/kg,
// rho_SR 2650 kg/m^3, nS 0.6). These are structural sample values shared
// with that test file, not literature-cited material parameters.
...
{
    double n_l = 0.3;
    double rho_lR = 1100.0;  // confined micro-liquid density scale (micro
                             // EOS; distinct from bulk free-water ~1000
                             // kg/m^3).
...
StrainedFilmStateData state(StrainedFilmSampleState const& st,
                            ...
                            FilmStrainKappaMode const kappa, double const eps_v,
                            double const p_conf)
{
...
TEST(RichardsMechanicsStrainedFilm, KinematicChainsFDConsistent)
{
         ...
         {FilmStrainKappaMode::Aggregate, FilmStrainKappaMode::Unity})
    {
        ...
        stm.n_l -= d;
        double const fd_dnl =
            (state(stp, FilmStrainCouplingMode::Kinematic, kappa, eps_v, p_conf)
                 .w_eff -
             state(stm, FilmStrainCouplingMode::Kinematic, kappa, eps_v, p_conf)
                 .w_eff) /
            (2.0 * d);
...
TEST(RichardsMechanicsStrainedFilm, EquilibriumInversionSolvesForceBalance)
{
    ...
    auto const s_loaded = state(st, FilmStrainCouplingMode::Equilibrium,
...
TEST(RichardsMechanicsStrainedFilm, EquilibriumInversionWithAugmentation)
{
    ...
    auto const s = state(st, FilmStrainCouplingMode::Equilibrium,
                         FilmStrainKappaMode::Aggregate, 0.0, p_loaded);
...
TEST(RichardsMechanicsStrainedFilm, ZeroStrainZeroLoadReducesToBareState)
{
                            ...
                            FilmStrainCouplingMode::Equilibrium})
    {
        ...
        EXPECT_DOUBLE_EQ(s.dw_eff_deps_v,
...
// log-linearly (getValueLogLinear) while the parse-time frozen-K path keeps
// the linear getValue(); the tests below anchor BOTH schemes accordingly.
// The knot values below are STRUCTURAL in-test constants (not physical
// material parameters); expected values are derived in-file from the
// respective interpolation identity.
namespace
{
PotentialExchangeParameters liveKSampleParams()
{
    ...
    params.micro_solid_density_reference =
        2650.0;  // rho_SR, mirrors the
                 // sample state above (kg/m^3).
...
// This test anchors the LINEAR interpolation scheme used by the
// parse-time frozen-K path (CreateRichardsMechanicsProcess.cpp,
// ->getValue(*dry_density)), which continues to call the table's LINEAR
// getValue() even though the LIVE free-function path
// (effectiveAugmentationPrefactor) interpolates log-linearly
// (getValueLogLinear); see LiveModeEvaluatesTableLogLinear below for the
// log-linear coverage. Physics anchor: linear-interpolation identity,
// derived in-file.
...
TEST(RichardsMechanicsLiveKOfRhoD, LiveModeEvaluatesTableLogLinear)
{
    ...
    // Interior points, derived in-file:
...
// exactly linear in phi, so the central FD is exact to roundoff;
// tolerances are derived in-file from the tangent scale. Knot values are
// STRUCTURAL in-test constants (not physical parameters), mirroring the
// live-K tests above.
// This test anchors the LINEAR segment-slope scheme
// (table.getSegmentSlope()) still used by the parse-time frozen-K path
// (CreateRichardsMechanicsProcess.cpp), even though the LIVE tangent
// effectiveAugmentationPrefactorPhiDerivative() uses the log-linear
// companion slope (getSegmentSlopeLogLinear). The live-path
// FD-vs-analytic, zero-case, and mu-level Jacobian-chain checks are in
// AnalyticPhiTangentMatchesFDLogLinear below. Physics anchor: linear
// segment-slope identity, derived in-file.
TEST(RichardsMechanicsLiveKOfRhoD,
     TableLinearSegmentSlopeAnchorsFrozenKParsePath)
{
    auto const params = liveKSampleParams();
    auto const& table = *params.potential_augmentation_prefactor_vs_dry_density;

    // Derived in-file: segment slope dK/drho_d = (30-10)/(2000-1000)
    // = 0.02 (J/kg)/(kg/m^3); chain dK/dphi = -rho_SR * slope (= -53,
    // formerly the live-path expectation before the log-linear switch,
    // preserved here as the LINEAR-scheme identity).
    ...
    EXPECT_NEAR(
        expected_dK_dphi,
        -params.micro_solid_density_reference * table.getSegmentSlope(1500.0),
        1e-12 * std::abs(expected_dK_dphi));
}

// Successor of the linear tangent test above, for the LIVE path:
// analytic-vs-FD self-consistency of
// effectiveAugmentationPrefactorPhiDerivative() under the log-linear
// tangent. Physics anchor (analytical derivation in-file): on the
// structural knots
// K(1000)=10, K(2000)=30,
...
TEST(RichardsMechanicsLiveKOfRhoD, AnalyticPhiTangentMatchesFDLogLinear)
{
    ...
    // Closed form derived in the comment above; tolerance 1e-12
    // relative, derived in-file from the phi round-trip noise exactly as
    // in LiveModeEvaluatesTableLogLinear.
    double const expected_dK_dphi = -50.425585997506346;
    EXPECT_NEAR(expected_dK_dphi, analytic, 1e-12 * std::abs(expected_dK_dphi));
    ...
    // roundoff. Sample state mirrors StrainedFilmSampleState
...
TEST(RichardsMechanicsLiveKOfRhoD, AnalyticPhiTangentClampedEdgesAndKnots)
{
    ...
    // derivative) uses the LOG-LINEAR pair; its tangents are derived
    // in-file below from the same knots.
    PotentialExchangeParameters params;
...
// true bit-for-bit rather than to ~1 ULP.
//
...
// "1400 low by 1 ULP, rel 1.6e-16"). Measured, before this test was added
// to the suite, by running getValueLogLinear/locateSegment/
// logLinearValueOnSegment extracted verbatim into a standalone program
// and compiled twice, with and without the t == 1 branch: both assertions
// below fail without it and pass with it, while the 10/16/30 assertions
...
{
    double const K_900 = 4367.227700212952;  // J/kg, historical dd900 knot
...
// derive from the FD step and the live-K table's piecewise-linear (C0)
// structure.
//
...
TEST(RichardsMechanicsLiveKOfRhoD,
     AssembledDisplacementTangentExactKinematicLiveK)
{
    ...
    // the dry density rho_d = rho_SR*(1-phi) in the table interior so dK/dphi
    // is a genuine (nonzero) segment slope. Structural constants;
    // material values mirror the tests in this file.
    PotentialExchangeParameters params;
    params.enabled = true;
    params.film_pressure_coupling = true;
    params.film_strain_coupling = FilmStrainCouplingMode::Kinematic;
    ...
    params.potential_augmentation_prefactor =
        20.0;  // structural scalar fallback
    ...
    // Pa, test value used elsewhere in this file.
    double const K_drained = 1.5e8;
    double const eps_v = -0.02;  // compression
    ...
    // (the live-K phi channel is part (B)).
    {
        ...
        double const analytic =
            g_cut0 * pr0.dmu_mech_deps_v;  // J/kg per strain
        double const h = 1e-7;
    ...
    // chain dphi/deps_v on the SAME eps_v step, n_l and the explicit-eps_v
    // drained line held), and compare to the M2 analytic chain
    // d(delta_sigma_sw)/dK * dK/dphi * dphi/deps_v.
    {
        // PorosityFromMassBalance dphi/deps_v = (alpha-phi)/(1+w); here on a
        // pure strain step w = delta_eps_v, alpha = biot. Interior -> no clamp.
        ...
        double const d_sigma_dK = (sigma_inc(K0 + dK) - sigma_inc(K0 - dK)) /
                                  (2 * dK);  // Pa per J/kg
        double const analytic =
            d_sigma_dK * dK_dphi * dphi_deps_v;  // Pa/strain
        ...
        double const K_plus =
            effectiveAugmentationPrefactor(params, phi0 + dphi_deps_v * he);
        double const K_minus =
            effectiveAugmentationPrefactor(params, phi0 - dphi_deps_v * he);
