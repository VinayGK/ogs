...
// The live K(rho_d) path uses the LOG-LINEAR pair:
...
public:
    ...
    // Flat-clamped outside [x_min, x_max] exactly like
    ...
    // (measured on an earlier version of the 900-knot table,
    // K(900)=4367.2277: the 1400 knot came out low by 1 ULP, rel -1.6e-16;
    // the table currently shipped happens to hit all four of its knots
    // bit-exactly, so this is a latent defect, not a live one).
    // logLinearValueOnSegment() below returns the
    ...
    // The non-positive cases do NOT share one failure mode: analysis of
    // logLinearValueOnSegment (IEEE-754 double) shows they are three
    // distinct chains, and only one of them starts at the logarithm:
...
// combination. Pure predicate so it is unit-testable; the OGS_FATAL lives at
// the parse site.
...
struct PotentialExchangeParameters
{
    ...
    // Value source: PRJ-supplied, not defaulted in code.
    // MANDATORY in the PRJ: the top-level
    // <potential_exchange> MUST declare <micro_water_content_floor>; the parser
    // no longer defaults it (see parsePotentialExchangeParameters). The 0.0
    // here is only the in-struct fallback for medium-override inheritance,
    ...
    // forced true at parse (CreateRichardsMechanicsProcess), so OFF is
    // unrunnable; the residual OFF code branches are dead and unused.
    bool film_pressure_coupling = true;
    // NOTE: the eigenstrain Biot b is NO LONGER a separate film parameter. It
    // is unified with the poroelastic biot_coefficient MPL medium property
    ...
    // Beyond the cap the film is saturated and further water stays bulk
    // (macro): porosity- and water-conserving (phi = phi_M + phi_m held; the
    // capped micro uptake remains in the macro mass balance). Value source:
    ...
    // -rho_SR*(K-linear segment slope) of the earlier wording, which
    // is a per-segment constant and no longer applies here (see
    // effectiveAugmentationPrefactorPhiDerivative below and
    // getSegmentSlopeLogLinear above).
...
//
// LOG-LINEAR SCHEME (production): evaluates the table via
// getValueLogLinear() -- ln(K) linear in rho_d between knots -- instead of
// the K-linear getValue() (which remains in use by the parse-time frozen-K
// path in CreateRichardsMechanicsProcess.cpp). Node values are unchanged (both
...
// REQUIRED so the residual and its Jacobian stay tangent-consistent; see
// getSegmentSlopeLogLinear doc for the chain-rule derivation). Chain
// (analytic derivation, this
