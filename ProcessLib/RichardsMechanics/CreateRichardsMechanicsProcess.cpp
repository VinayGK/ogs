...
LocalNonlinearSolveMode parseLocalNonlinearSolveMode(std::string const& mode)
{
...
MacroPorosityUpdateMode parseMacroPorosityUpdateMode(std::string const& mode)
{
...
{
    //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__enabled}
    auto const enabled = config.getConfigParameter<bool>(
        "enabled", defaults ? defaults->enabled : false);

    //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__pressure_tolerance}
    auto const pressure_tolerance = config.getConfigParameter<double>(
        "pressure_tolerance", defaults ? defaults->pressure_tolerance : 0.0);
    ...
    auto const micro_potential_convention =
        //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__micro_potential_convention}
        parseMicroPotentialConvention(config.getConfigParameter<std::string>(
            "micro_potential_convention",
            defaults ? toString(defaults->micro_potential_convention)
                     : "positive_reduced"));
    auto const local_nonlinear_solve_mode =
        //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__local_nonlinear_solve_mode}
        parseLocalNonlinearSolveMode(config.getConfigParameter<std::string>(
            "local_nonlinear_solve_mode",
            defaults ? toString(defaults->local_nonlinear_solve_mode)
                     : "scalar_exchange"));
    auto const macro_porosity_update_mode =
        //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__macro_porosity_update_mode}
        parseMacroPorosityUpdateMode(config.getConfigParameter<std::string>(
    ...
    auto const micro_solid_volume_fraction_mode =
        parseMicroSolidVolumeFractionMode(
            //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__micro_solid_volume_fraction_mode}
            config.getConfigParameter<std::string>(
                "micro_solid_volume_fraction_mode",
                defaults ? toString(defaults->micro_solid_volume_fraction_mode)
                         : "reference"));
        ...
        [&](char const* const key, double const fallback)
    {
        ...
        if (!(selected > 0.0))
        {
            OGS_FATAL("RichardsMechanics: {} {} must be > 0, got {:g}.",
    ...
    auto get_positive_optional_or_default =
        [&](char const* const key,
            std::optional<double> const fallback) -> std::optional<double>
    {
        //! \ogs_file_special
        auto const value = config.getConfigParameterOptional<double>(key);
        std::optional<double> selected =
            value ? std::optional<double>{*value} : fallback;
    ...
    //! \ogs_file_param_special{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__hamaker_constant}
    //! \ogs_file_param_special{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__specific_surface}
    //! \ogs_file_param_special{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__micro_solid_density_reference}
    //! \ogs_file_param_special{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__micro_solid_volume_fraction_reference}
    //! \ogs_file_param_special{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__micro_liquid_density_reference}
    //! \ogs_file_param_special{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__micro_liquid_density_a}
    //! \ogs_file_param_special{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__micro_liquid_density_b}
    //! \ogs_file_param_special{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__initial_micro_water_content}
    double const default_hamaker = defaults ? defaults->hamaker_constant : 0.0;
    double const default_surface = defaults ? defaults->specific_surface : 0.0;
    ...
    if (enabled)
    {
        hamaker_constant = get_positive_required_or_default("hamaker_constant",
                                                            default_hamaker);
        specific_surface = get_positive_required_or_default("specific_surface",
                                                            default_surface);
        ...
        {
            micro_liquid_density_reference = get_positive_required_or_default(
        ...
        else
        {
            micro_liquid_density_reference =
                get_positive_optional_or_default(
                    ...
                    defaults
                        ? std::optional<
                              double>{defaults->micro_liquid_density_reference}
                        : std::nullopt)
                    .value_or(0.0);
    ...
    else
    {
        hamaker_constant =
            get_positive_optional_or_default(
                "hamaker_constant",
                defaults ? std::optional<double>{defaults->hamaker_constant}
                         : std::nullopt)
                .value_or(0.0);
        specific_surface =
            get_positive_optional_or_default(
                "specific_surface",
                defaults ? std::optional<double>{defaults->specific_surface}
                         : std::nullopt)
                .value_or(0.0);
    ...
    //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__fd_jacobian_for_exchange}
    ...
    //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__fd_jacobian_perturbation}
    ...
    {
        OGS_FATAL(
            "RichardsMechanics: {} fd_jacobian_perturbation must be > 0, got "
            "{:g}.",
            context, fd_jacobian_perturbation);
    }

    //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__local_jacobian_perturbation}
    ...
    {
        OGS_FATAL(
            "RichardsMechanics: {} local_jacobian_perturbation must be > 0, "
            "got {:g}.",
    ...
    // in time (initial/target rho_d) so no Jacobian term is
    ...
    std::shared_ptr<AugmentationPrefactorTable const>
        potential_augmentation_prefactor_vs_dry_density =
            defaults ? defaults->potential_augmentation_prefactor_vs_dry_density
                     : nullptr;
    //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__potential_augmentation_prefactor_vs_dry_density}
    if (auto k_curve_config = config.getConfigSubtreeOptional(
            "potential_augmentation_prefactor_vs_dry_density"))
    {
        auto dry_densities =
            //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__potential_augmentation_prefactor_vs_dry_density__dry_densities}
            k_curve_config->getConfigParameter<std::vector<double>>(
                "dry_densities");
        auto prefactors =
            //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__potential_augmentation_prefactor_vs_dry_density__prefactors}
            k_curve_config->getConfigParameter<std::vector<double>>(
                "prefactors");
        ...
        // that chain in three different places -- confirmed in a
    ...
    std::optional<double> dry_density =
        //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__dry_density}
    ...
    auto const potential_augmentation_prefactor_live_dry_density =
        ...
        config.getConfigParameter<bool>(
            ...
            defaults
                ...
                : false);
    //! \ogs_file_param_special{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__potential_augmentation_prefactor_live_dry_density}
    ...
    auto const potential_augmentation_prefactor_scalar =
        //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__potential_augmentation_prefactor}
    ...
    {
        OGS_FATAL(
            "RichardsMechanics: {} potential_augmentation_prefactor must be >= "
            "0, got {:g}.",
            context, potential_augmentation_prefactor);
    }

    auto const potential_augmentation_exponent =
        //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__potential_augmentation_exponent}
    ...
    {
        OGS_FATAL(
            "RichardsMechanics: {} potential_augmentation_exponent must be > 0 "
            "when "
    ...
    auto const use_micro_liquid_density_for_micro_pressure =
        ...
        config.getConfigParameter<bool>(
            ...
            defaults ? defaults->use_micro_liquid_density_for_micro_pressure
                     : true);
    //! \ogs_file_param_special{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__use_micro_liquid_density_for_micro_pressure}
    if (!use_micro_liquid_density_for_micro_pressure)
    {
        WARN(
            "RichardsMechanics: {} "
            "use_micro_liquid_density_for_micro_pressure=false selected; micro "
            "pressure will use bulk rho_LR instead of confined rho_lR.",
            context);
    }
    ...
    // warning.
    //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__film_pressure_coupling}
    ...
    //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__film_pressure_gate_width}
    ...
    {
        OGS_FATAL(
            "RichardsMechanics: {} film_pressure_gate_width must be >= 0, got "
            "{:g}.",
            context, film_pressure_gate_width);
    }
    // Deprecated: swelling stress is now (1-phi_M)*p_film; this
    // modulus is unused.
    auto const film_pressure_swelling_modulus =
        //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__film_pressure_swelling_modulus}
        config.getConfigParameter<double>(
    ...
    {
        OGS_FATAL(
            "RichardsMechanics: {} film_pressure_swelling_modulus must be >= "
            "0, got {:g}.",
    ...
    auto const film_strain_coupling =
        ...
        parseFilmStrainCouplingMode(config.getConfigParameter<std::string>(
            "film_strain_coupling",
            defaults ? toString(defaults->film_strain_coupling) : "off"));
    auto const film_strain_kappa =
        //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__film_strain_kappa}
        parseFilmStrainKappaMode(config.getConfigParameter<std::string>(
            "film_strain_kappa",
    ...
    auto const film_energy_route =
        //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__film_energy_route}
        parseFilmEnergyRoute(config.getConfigParameter<std::string>(
            "film_energy_route",
    ...
    // MANDATORY: like micro_water_content_floor, the
    // top-level <potential_exchange> MUST declare macro_porosity_floor; the
    // parser no longer defaults it (per-medium overrides inherit). An explicit
    // 0.0 (= no floor) is permitted but must be a conscious declaration.
    ...
    //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__macro_floor_cutoff_width}
    ...
    {
        OGS_FATAL(
            "RichardsMechanics: {} macro_floor_cutoff_width must be >= 0, got "
            "{:g}.",
            context, macro_floor_cutoff_width);
    }

    // Disjoining-pressure floor via a micro-water-content lower bound: clamps
    // the water content used in the vdW disjoining law so Pi = Pi(max(n_l,
    // floor)), capping Pi instead of diverging as n_l -> 0. MANDATORY:
    // the floor MUST be declared in every top-level
    // <potential_exchange> block. A silently-absent floor let Pi ~ 1/n_l^3
    // diverge and detonate the micro solve during Tuller-model resaturation.
    // Per-medium overrides inherit the top-level value. An explicit 0.0 is
    // still permitted
    // (= no floor) but must now be a conscious declaration, not a default.
    auto const micro_water_content_floor =
        defaults
            //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__micro_water_content_floor}
            ? config.getConfigParameter<double>(
    ...
    std::map<int, std::shared_ptr<MaterialPropertyLib::Medium>> const& media)
{
        ...
        config.getConfigSubtreeOptional("potential_exchange"))
    {
        potential_exchange_parameters = parsePotentialExchangeParameters(
            *potential_exchange_config, std::nullopt, "potential_exchange");

        for (
            auto medium_config :
            //! \ogs_file_param{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__medium}
            potential_exchange_config->getConfigSubtreeList("medium"))
        {
            //! \ogs_file_attr{prj__processes__process__RICHARDS_MECHANICS__potential_exchange__medium__id}
            ...
            if (!potential_exchange_parameters_by_material
                     .emplace(material_id,
                              parsePotentialExchangeParameters(
                                  medium_config,
                                  potential_exchange_parameters,
                                  fmt::format("potential_exchange medium id {}",
                                              material_id)))
                     .second)
            {
                OGS_FATAL(
                    "RichardsMechanics: duplicate potential_exchange medium "
                    "override for material id {}.",
                    material_id);
            }
        }
    }
