...
std::vector<DSMMicroMacroOverlapBaselineRow>
loadDSMMicroMacroOverlapBaselineRows(std::string const& filename)
{
    ...
    while (std::getline(in, line))
    {
        ...
        rows.push_back(
            {.step = static_cast<int>(get_value(fields, "step")),
             ...
             .phi_M = get_value(fields, "phi_M"),
             .phi = get_optional_value(
                 fields, "phi",
                 get_value(fields, "phi_m") + get_value(fields, "phi_M")),
...
double comparisonTolerance(double const a, double const b,
                           double const rel = 1e-10, double const abs = 1e-14)
{
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
    auto const solve_rho = [&](double const n_eval)
    {
        ...
        auto const rhs = [&](double const rho_lR)
        {
            ...
            return std::pair{
                omega_l,
                rho_l0 * std::exp(-a_rho * std::pow(omega_l, b_rho)) + rho_LR};
        };

        double rho_lR =
            rho_LR +
            rho_l0 *
                std::exp(-a_rho * std::pow(std::max(1e-16, n_l_safe * rho_LR /
                                                               denominator),
                                           b_rho));
        ...
        for (int iter = 0; iter < max_iterations; ++iter)
        {
            ...
            if (std::abs(residual) <= 1e-14 * std::max(1.0, std::abs(rho_lR)))
            {
    ...
    double const volumetric_strain_prev = 0.0)
{
    ...
    double const phi_safe = std::isfinite(phi)
    ...
    if (use_mass_storage)
    {
        ...
        auto const split = computeTransportPorosityUpdate(
            phi_safe, std::max(0.0, phi_safe - n_l_prev),
            std::max(0.0, n_l_prev), coupled_update.n_l, volumetric_strain,
            volumetric_strain_prev,
            potential_exchange_params.macro_porosity_update_mode);
        ...
        return {
            ...
            .p_L_m =
                -coupled_update.rho_lR * coupled_update.micro_potential.mu_lR,
    ...
    auto const residual = [&](double const n_l)
    {
        ...
        if (use_mass_storage)
        {
            ...
            double residual = rho_l - rho_l_prev - dt * exchange.rho_l_hat;
            residual -= dt * rho_l * volumetric_strain_rate;
            return residual;
        }
        ...
        potential_exchange_params.use_micro_liquid_density_for_micro_pressure)
    {
        double const active_nS = referenceMicroSolidVolumeFraction(
            n_l, phi, 0.0, n_l_prev, volumetric_strain, volumetric_strain_prev,
            potential_exchange_params);
        rho_lR_output = solveReferenceReducedMicroLiquidDensity(
                            n_l, rho_LR, active_nS, potential_exchange_params)
                            .rho_lR;
    }
    double const n_l_ref = std::max(
    ...
    double const volumetric_strain_prev = 0.0)
{
    ...
    auto const plus = solveDsmMicromacroReferenceSinglePoint(
        p_L + h, n_l_prev, dt, rho_LR, alpha_bar, mu, phi,
        potential_exchange_params, volumetric_strain, volumetric_strain_prev);
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
    ...
    if (state.mode == CoupledExchangeReferenceMode::pressure_proxy)
    {
        double const alpha_M_effective =
            state.alpha_bar * rho_LR_eval / state.mu;
    ...
    auto const reference = solveDsmMicromacroReferenceSinglePoint(
        ...
        state.volumetric_strain, state.volumetric_strain_prev);
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
    ...
    double const plus = referenceCoupledRhoLHat(state, state.p_L + h,
                                                potential_exchange_params);
    double const minus = referenceCoupledRhoLHat(state, state.p_L - h,
                                                 potential_exchange_params);
    ...
    PotentialExchangeParameters const& potential_exchange_params)
{
    ...
    if (state.mode == CoupledExchangeReferenceMode::pressure_proxy)
    {
        auto const data = computePotentialExchangeUpdate(
            ...
            std::numeric_limits<double>::quiet_NaN(), state.pressure_tolerance,
            false, false, 0.0, 0.0, false, 0.0, false,
    ...
    double const dn_l_dpL = computeImplicitNlDpL(
        state.n_l_prev, state.p_L, state.dt, state.rho_LR, state.drho_LR_dpL,
        state.alpha_bar, state.mu, macro_potential, n_l_update.micro_potential,
        n_l_update.exchange,
        {.phi = state.phi,
         .volumetric_strain = state.volumetric_strain,
         .volumetric_strain_prev = state.volumetric_strain_prev},
    ...
    auto const data = computePotentialExchangeUpdate(
        ...
        beta_LR, std::numeric_limits<double>::quiet_NaN(),
        std::numeric_limits<double>::quiet_NaN(), state.pressure_tolerance,
        true, true, n_l_update.micro_potential.mu_lR,
        n_l_update.micro_potential.dmu_lR_drho_lR, true, dmu_lR_vdw_dpL, false,
        potential_exchange_params.fd_jacobian_perturbation);
...
TEST(RichardsMechanics, DSMMicroMacroSingleIntegrationPointReferencePath)
{
    ...
    auto const reference = solveDsmMicromacroReferenceSinglePoint(
        p_L, n_l_prev, dt, rho_LR, alpha_bar, mu, phi,
        potential_exchange_params);

    EXPECT_NEAR(ogs_update.n_l, reference.n_l,
                comparisonTolerance(ogs_update.n_l, reference.n_l));
    EXPECT_NEAR(
        ogs_update.micro_potential.mu_lR, reference.micro_potential.mu_lR,
        comparisonTolerance(ogs_update.micro_potential.mu_lR,
                            reference.micro_potential.mu_lR, 1e-10, 1e-18));
    EXPECT_NEAR(
        ogs_update.exchange.rho_l_hat, reference.exchange.rho_l_hat,
        comparisonTolerance(ogs_update.exchange.rho_l_hat,
                            reference.exchange.rho_l_hat, 1e-10, 1e-18));
    EXPECT_NEAR(
        ogs_update.exchange.rho_L_hat, reference.exchange.rho_L_hat,
        comparisonTolerance(ogs_update.exchange.rho_L_hat,
                            reference.exchange.rho_L_hat, 1e-10, 1e-18));
    ...
    EXPECT_NEAR(compatibility_output.p_L_m, reference.p_L_m,
                comparisonTolerance(compatibility_output.p_L_m, reference.p_L_m,
                                    1e-10, 1e-12));
    EXPECT_NEAR(
        compatibility_output.S_L_m, reference.S_L_m,
        comparisonTolerance(compatibility_output.S_L_m, reference.S_L_m));
    ...
    EXPECT_NEAR(
        transport_porosity_update.phi_M, reference.phi_M,
        comparisonTolerance(transport_porosity_update.phi_M, reference.phi_M));
    EXPECT_NEAR(
        transport_porosity_update.phi_m, reference.phi_m,
        comparisonTolerance(transport_porosity_update.phi_m, reference.phi_m));
    EXPECT_NEAR(transport_porosity_update.phi_M_prev, phi_prev - n_l_prev,
                comparisonTolerance(transport_porosity_update.phi_M_prev,
                                    phi_prev - n_l_prev));
    EXPECT_NEAR(
        transport_porosity_update.phi_m_prev, n_l_prev,
        comparisonTolerance(transport_porosity_update.phi_m_prev, n_l_prev));
    ...
    double const analytic_dn_l_dpL = computeImplicitNlDpL(
        ...
        {.phi = phi, .volumetric_strain = 0.0, .volumetric_strain_prev = 0.0},
        potential_exchange_params);
    double const reference_dn_l_dpL =
        referenceDnLDpL(p_L, n_l_prev, dt, rho_LR, alpha_bar, mu, phi,
                        potential_exchange_params);

    EXPECT_NEAR(analytic_dn_l_dpL, reference_dn_l_dpL,
                comparisonTolerance(analytic_dn_l_dpL, reference_dn_l_dpL, 5e-5,
                                    1e-18));
}

TEST(RichardsMechanics,
     DSMMicroMacroBranchSensitivityNearMacroPotentialTransition)
{
    PotentialExchangeParameters potential_exchange_params;
    potential_exchange_params.enabled = true;
    ...
    for (std::size_t i = 0; i < pressures.size(); ++i)
    {
        ...
        EXPECT_LE(ogs_update.n_l,
                  n_l_prev + comparisonTolerance(ogs_update.n_l, n_l_prev, 0.0,
                                                 1e-18));
...
TEST(RichardsMechanics,
     DSMMicroMacroNegativeAttractiveMicroPotentialAdmitsWetting)
{
    PotentialExchangeParameters potential_exchange_params;
    potential_exchange_params.enabled = true;
...
TEST(RichardsMechanics, DSMMicroMacroMicroPorositySwellingStressIncrement)
{
    ...
    //
    //   Pi_tau            = -p_L_m_density_tau * mu_lR_tau            (FULL
    //   p^disj) p_L_m_density_tau = use_micro_liquid_density_for_micro_pressure
    //                           ? rho_lR_tau : rho_LR
    //   mu_lR_tau         = sign * (mu_lR_vdW_tau + mu_lR_aug_tau)    (FULL
    //   potential)
    //
    ...
    auto const expected_increment =
        [&](double const n_l_prev, double const n_l, double const n_S,
            double const rho_lR_curr, double const rho_lR_prev,
            double const rho_LR)
    {
...
TEST(RichardsMechanics, DSMMicroMacroSwellingStressFullDisjoiningSign)
{
    ...
    auto const expected_Pi =
        [&](double const n_l_eval, double const rho_lR_eval)
    {
        double const active_nS = computeActiveMicroSolidVolumeFraction(
            n_l_eval, PotentialExchangeLocalSolveContext{},
            potential_exchange_params);
...
TEST(RichardsMechanics,
     DSMMicroMacroTransportPorositySplitRecomposesTotalPorosity)
{
    auto const split =
        computeTransportPorosityUpdate(0.4, 0.27, 0.08, 0.1, 0.0, 0.0,
                                       MacroPorosityUpdateMode::AlgebraicSplit);

    // Hierarchical split: phi_M = (phi - n_l) / (1 - n_l), phi_m = (1 - phi_M)
    // * n_l For phi=0.4, n_l=0.1: phi_M = 0.3/0.9 = 1/3, phi_m = (2/3)*0.1 =
    // 1/15
    ...
    auto const clamped = computeTransportPorosityUpdate(
        0.25, 0.0, 0.2, 0.4, 0.0, 0.0, MacroPorosityUpdateMode::AlgebraicSplit);
...
TEST(RichardsMechanics, DSMMicroMacroAdditiveMacroPorosityRateUpdate)
{
    ...
    auto const split = computeTransportPorosityUpdate(
        ...
        volumetric_strain_prev, MacroPorosityUpdateMode::ReferenceAdditiveRate);
    ...
    double const phi_safe_kinematic =
        (phi_prev_sum + delta_eps_v) / denominator;

    // Hierarchical split: phi_M = (phi_safe - n_l) / (1 - n_l), phi_m = (1 -
    // phi_M) * n_l n_l here is the phi_m argument passed to
...
TEST(RichardsMechanics, DSMMicroMacroCurrentPorositySplitMicroSolidFractionMode)
{
    ...
    auto const active_nS = computeActiveMicroSolidVolumeFraction(
        n_l, local_context, potential_exchange_params);
    // active_nS = 1 - n_l (aggregate micro-solid volume fraction): the
    // disjoining potential uses (1 - n_l), NOT (1 - phi_M).
    // CurrentPorositySplit
    // mode uses only n_l and ignores the porosity context. An earlier
    ...
    // reproduce the old 3-argument overload behaviour (since removed).
    ...
    double const expected_ratio = std::pow(
        active_nS /
            potential_exchange_params.micro_solid_volume_fraction_reference,
        3.0);
...
TEST(RichardsMechanics, DSMMicroMacroReducedMicroLiquidDensityEOSReferencePath)
{
    ...
    auto const production = computeReducedMicroLiquidDensity(
        n_l, rho_LR, nS, potential_exchange_params);
    auto const reference = solveReferenceReducedMicroLiquidDensity(
        n_l, rho_LR, nS, potential_exchange_params);

    EXPECT_NEAR(
        production.rho_lR, reference.rho_lR,
        comparisonTolerance(production.rho_lR, reference.rho_lR, 1e-9, 1e-14));
    EXPECT_NEAR(production.omega_l, reference.omega_l,
                comparisonTolerance(production.omega_l, reference.omega_l, 1e-9,
                                    1e-14));
...
TEST(RichardsMechanics, DSMMicroMacroScalarStorageLocalSolveReferencePath)
{
    ...
    EXPECT_LE(scalar_update.n_l,
              phi + comparisonTolerance(scalar_update.n_l, phi));
...
TEST(RichardsMechanics, DSMMicroMacroScalarMassStorageLocalSolveReferencePath)
{
    ...
    // computeYoungLaplaceMacroPotential is C0 (dmu_LR/dp_L jumps 1/rho_LR ->
    // 0): a central-difference FD reference straddling the kink returns ~half
    // the one-sided tangent, and the FD step h ~ 1e-8*|p_L| collapses to 1e-8
    // there (catastrophic cancellation). Both pathologies are artifacts of the
    // probe point, not of computeImplicitNlDpL; probe at a genuine unsaturated
    // state (1 MPa suction) where the residual is smooth in p_L and h ~ 1e-2
    // keeps the FD well-conditioned. The probe state is deliberately chosen as
    // an unsaturated Gauss point, not the saturation kink.
    ...
    EXPECT_NEAR(
        ogs_update.n_l, reference_n_l,
        comparisonTolerance(ogs_update.n_l, reference_n_l, 1e-8, 1e-14));
    ...
    auto const reference_dn_l_dpL = [&]()
    {
        ...
        auto const eval_n_l = [&](double const p_eval)
        {
            ...
            return solveImplicitMicroWaterContent(
                       n_l_prev, dt, rho_LR, alpha_bar, mu,
    ...
    EXPECT_NEAR(analytic_dn_l_dpL, reference_dn_l_dpL,
                comparisonTolerance(analytic_dn_l_dpL, reference_dn_l_dpL, 1e-6,
                                    1e-12));
}
...
TEST(RichardsMechanics, DSMMicroMacroMassStorageCoupledSolveResiduals)
{
    ...
    auto const macro_potential = computeYoungLaplaceMacroPotential(
        p_L, rho_LR, potential_exchange_params.pressure_tolerance);
    auto const prev_micro_liquid_density = computePreviousMicroLiquidDensity(
    ...
    auto const coupled_update = solveReferenceMassStorageCoupledState(
        ...
        alpha_bar, mu, macro_potential, local_context,
        potential_exchange_params);
    ASSERT_TRUE(coupled_update.converged);
    EXPECT_GT(coupled_update.n_l, 0.0);
    EXPECT_LE(coupled_update.n_l,
              phi + comparisonTolerance(coupled_update.n_l, phi, 1e-10, 1e-12));
    ...
    auto const exchange = computePotentialDrivenMassExchange(
        alpha_bar * rho_LR / mu, macro_potential.mu_LR, micro_potential.mu_lR);

    double const one_minus_n_l = std::max(1e-12, 1.0 - coupled_update.n_l);
    double const rho_l = (1.0 - std::clamp(phi, 0.0, 1.0 - 1e-12)) /
                         one_minus_n_l * coupled_update.n_l *
                         coupled_update.rho_lR;
    double const volumetric_strain_rate =
        (volumetric_strain - volumetric_strain_prev) / dt;
    double const mass_residual = rho_l - rho_l_prev - dt * exchange.rho_l_hat -
...
TEST(RichardsMechanics, DSMMicroMacroOverlapTransferBaselineHistory)
{
    ...
    PotentialExchangeParameters potential_exchange_params;
    potential_exchange_params.enabled = true;
    // Match the shared dsm_micromacro/MFront nonnegative branch: p_L = 0
    // belongs to the saturated Young-Laplace side.
    ...
    for (auto const& row : baseline_rows)
    {
        ...
        auto const n_l_update = solveImplicitMicroWaterContent(
            n_l_prev, dt, rho_LR, alpha_bar, mu, macro_potential, local_context,
            potential_exchange_params);
        ...
        auto const rho_lR_update = computeActiveMicroLiquidDensity(
                                       n_l_update.n_l, rho_LR, local_context,
                                       potential_exchange_params)
                                       .rho_lR;
...
TEST(RichardsMechanics, DSMMicroMacroStrainCoupledOverlapBaselineHistory)
{
    ...
    for (auto const& row : strain_rows)
    {
        ...
        epsilon_sw += delta_epsilon_sw;
        double const bulk_modulus = E / (3.0 * (1.0 - 2.0 * nu));
        double const sigma_xx = sigma_xx_prev +
                                bulk_modulus * row.delta_epsilon_v -
...
TEST(RichardsMechanics, PotentialExchangeTangentRepresentativeStates)
{
    ...
    {
        auto const reference_rho_L_hat = referenceCoupledRhoLHat(
            state, state.p_L, potential_exchange_params);
        auto const reference_drho_L_hat_dpL =
            referenceCoupledDrhoLHatDpL(state, potential_exchange_params);
        auto const production =
            productionCoupledExchangeData(state, potential_exchange_params);
...
//   so when the gate is open (g = 1) and saturated, mu_lR = -(Pi -
//   b*p_conf)/rho_lR.
...
//
// CRITICAL two-nS distinction: the vdW core consumes the
...
{
    // Gate-open state: p_conf >> Pi_gate with a finite smooth-gate width, so
    // the smoothstep saturates (g = 1, dg/dx = 0). With g pinned at 1 the film
    // delta D = b*p_conf/rho_lR and its p_conf- and rho_lR-tangents are exact,
    // so the central finite differences must match the analytic partials to
    // round-off.
    ...
    // only uses it inside dPi_gate/dn_l (the n_l-partial), which is multiplied
    // by dg/dx = 0 in the gate-saturated region, so it does NOT affect the two
    ...
    // Central FD of mu_lR_film_delta w.r.t. rho_lR == dmu_lR_film_drho_lR.
    {
        ...
        double const fd = (plus - minus) / (2.0 * h);
        EXPECT_NEAR(
            fd, film.dmu_lR_film_drho_lR,
            1e-12 + 1e-6 * std::max(std::abs(fd),
                                    std::abs(film.dmu_lR_film_drho_lR)));
    }
    // NOTE: we deliberately do NOT FD over n_l. The helper's dmu_lR_film_dnl is
    // the gate-threshold partial at FIXED Pi; a real n_l perturbation would
    // also move Pi (Pi = Pi(n_l)), so the in-helper partial is not a total
    // derivative and is out of scope for a single-argument FD here.
}
...
TEST(RichardsMechanics, DSMFilmPressureDrain)
{
    ...
    // n_S_agg = 1 - phi_M (see the two-nS distinction noted above).
    ...
    std::array<double, 5> const p_conf_seq = {0.0, Pi_gate, 2.0 * Pi_gate,
                                              5.0e3, 5.0e4};
    ...
    // (i) Non-decreasing across the WHOLE sweep: rising confinement never
    ...
    // mu_lR > mu_LR (so rho_l_hat < 0); uptake at p_conf = 0 needs mu_lR <
    // mu_LR.
    ...
    {
        auto const exch = computePotentialDrivenMassExchange(alpha_M_eff, mu_LR,
                                                             mu_lR_seq[0]);
...
TEST(RichardsMechanics, DSMFilmEigenstressSignDrains)
{
    ...
    // Flag-ON film-pressure swelling stress (MICRO-WEIGHTED PRESSURE
    // form): the micro swelling stress is a transmitted
    ...
    //   n_l*p_film_curr)*identity2, p_film         = Pi - b*p_conf (p_conf HELD
    //   FIXED across the step).
    // Pi is the BARE van-der-Waals disjoining pressure Pi = -rho * mu_vdW. K_sw
    // is GONE. With the n_l weighting the -b*p_conf term does NOT cancel in the
    // increment (the weights n_l_prev, n_l differ); the NaN-p_conf default
    // drops the drain (p_conf -> 0).
    //
    // This re-derives the expected increment from the SAME bare-vdW Pi the
    // production residual recomputes (independent reconstruction here, so this
    // is a genuine cross-check of the assembled increment, not a
    // fit-and-verify), then confirms the n_l-derivative carries the formula's
    // sign.
    PotentialExchangeParameters potential_exchange_params;
    potential_exchange_params.enabled = true;
    ...
    double const n_S =
        0.63;  // = 1 - phi_M (REV macro-solid / contact-area factor)
    double const b =
        1.0;  // Biot b passed explicitly (cancels in the increment)
    double const rho_lR = 1000.0;
    double const rho_lR_prev = 1000.0;
    double const rho_LR = 1000.0;
    double const n_l_prev = 0.1;
    ...
    // BARE vdW disjoining pressure Pi = -density * mu_vdW, density mirrored to
    // the residual's p_L_m choice (micro rho_lR when enabled, here == rho_lR).
    ...
    //   p_film = Pi - b*p_conf, p_conf held fixed (current value for both
    //   terms).
    ...
    // = 1000 => bare Pi ~ O(10) Pa) and no augmentation, so phi_m*Pi =
    // n_S*n_l*Pi ~ 1/n_l^2 actually DECREASES with wetting here, and at p_conf
    // = 8 MPa the -b*p_conf drain dominates the increment entirely. The
    // increment sign is therefore formula-dependent, NOT a universal
    // "compressive on wetting" -- it is verified against the analytic slope in
    // (3) below (whatever-the-formula- gives), and the physical swelling-sign /
    // density gate is exercised on the REAL models (constant-volume
    // dd1400/dd1600, full Sa + augmentation), not in this synthetic GP unit.

    // (2) p_conf drain does NOT cancel under the n_l weighting: the
    // finite-p_conf call and the NaN-default (drain dropped, p_conf -> 0) call
    // differ by exactly
    ...
    // WHATEVER the formula gives (no assumed swelling sign), via central FD of
    // the assembled mean against the independently reconstructed analytic
    // slope. Uses the NaN-default call (drain dropped), so p_film here is the
    // BARE Pi.
    ...
    double const Pi_at_nl =
        bare_Pi(n_l, rho_lR);  // p_film = Pi (drain dropped)
...
TEST(RichardsMechanics, DSMIntegrableMechanicalPotentialTangents)
{
    ...
    // derivative (spec item 2): it bakes in dPi/dn_l = Pi' and dPi'/dn_l =
    ...
    // FD the total derivative through it. Calibration-blind: the model is
    // synthetic and asserted only against the helper's own analytic value
    // (round-off).
    {
        ...
        double const h = 1e-7;
        double const plus = computeIntegrableMechanicalMicroPotential(
                                Pi_model(n_l + h), dPi_model(n_l + h),
                                d2Pi_dnl2, n_l + h, eps_v, b, K_drained, rho_lR)
                                .mu_lR_mech;
        double const minus =
            computeIntegrableMechanicalMicroPotential(
                Pi_model(n_l - h), dPi_model(n_l - h), d2Pi_dnl2, n_l - h,
                eps_v, b, K_drained, rho_lR)
                .mu_lR_mech;
        double const fd = (plus - minus) / (2.0 * h);
...
// (computeReferenceMicroPorositySwellingStressIncrement, the current
// (provisional) residual form
// sigma_sw = -phi_m*p_film, p_conf set on the drained line); RHS is the
// analytic d mu_lR_mech/d eps_v from the integrable partner, built from the
// SAME bare-vdW Pi, Pi'. NO expected value: both sides are computed from the
// implementation; the test asserts they agree (integrability). Verifies spec
// item (3).
TEST(RichardsMechanics, DSMMaxwellPairIntegrabilityIdentity)
{
    ...
    auto const& identity2 = MathLib::KelvinVector::Invariants<
        MathLib::KelvinVector::kelvin_vector_dimensions(2)>::identity2;
    double const sign =
        microPotentialSignFactor(params.micro_potential_convention);
    ...
    // RHS: n_S * rho_lR * d mu_lR/d eps_v from the integrable partner, built
    // from the SAME bare-vdW Pi, Pi' (Pi'' via FD here; it does not enter the
    // eps_v partial). d mu_lR/d eps_v = d mu_lR_mech/d eps_v (vdW base
    // eps_v-independent).
    ...
    ASSERT_NE(rhs, 0.0);
    EXPECT_NEAR(lhs, rhs, 1e-3 + 1e-5 * std::max(std::abs(lhs), std::abs(rhs)));
