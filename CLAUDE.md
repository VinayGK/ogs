...
3. **Never delete .md files.** If content is superseded, annotate as
   ...
   - **They must stay under git control.** CLAUDE.md, every AGENTS.md,
     and all guardrail/worklog `.md` files MUST be tracked. Never add
     them to `.gitignore`; if a pattern (e.g. `CLAUDE.md`, `AGENTS.md`,
     `AGENTS_*.md`) ever excludes them, remove that pattern and re-add
     the files. These files may only be *cleaned up* (trimmed, reflowed,
     annotated historical) — never removed from disk or from the index.
