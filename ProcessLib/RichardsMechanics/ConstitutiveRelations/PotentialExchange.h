...
struct VanDerWaalsMicroPotentialData
{
    ...
    // PARTIAL second derivative d^2 mu_lR/d n_l^2 (nS held fixed), needed by
    // the INTEGRABLE Maxwell mechanical partner (mu_lR_mech, below) for its
    // analytic n_l-derivative via Pi'' = -rho_lR*d^2 mu_lR_vdw/d n_l^2.
    // Analytic, derived in this file from the cubic core (12*mu/n_l^2) plus
    // augmentation (mu_aug*(xi/n_l)^2). The live-nS chain is NOT threaded
    // into this second partial (tangent-only path; the integrable partner's
    // n_l-tangent is used in the analytic predictor/scalar micro-solve
    // Jacobians only).
    double d2mu_lR_dnl2 = 0.0;
    // PARTIAL derivatives w.r.t. the augmentation prefactor K (all other
    // state held fixed), needed by the live-K(rho_d) Jacobian chain. Every
    // K-channel below is LINEAR in K, so these are exact:
    ...
    double const dnS_dnl = 0.0, double const n_l_floor = 0.0)
{
    ...
    {
        OGS_FATAL(
            "computeVanDerWaalsMicroPotential requires n_l > 0, got {:g}.",
            n_l);
    }
    if (!(rho_lR > 0.0))
    {
    ...
    // -> 0). With the default n_l_floor = 0 the OGS_FATAL above already
    // enforced n_l > 0, so clamped == false and n_l_eff == n_l exactly
    // (byte-identical).
    ...
    // clamped omega_l is constant in n_l). Unclamped: rho_lR/(nS*rho_SR) as
    // before.
    ...
    // below via the dnS_dnl multiplier (tangent-only).
    //
    ...
    // PARTIAL second derivative of the cubic core (mu_core ~ C/n_l^3, nS
    // fixed):
    ...
    if (potential_augmentation_prefactor > 0.0)
    {
        ...
        double const mu_aug = potential_sign_factor *
                              potential_augmentation_prefactor * std::exp(-xi);
        ...
        // PARTIAL second derivative of the augmentation (nS fixed): xi linear
        // in n_l so xi/n_l = const, d mu_aug/d n_l = -mu_aug*(xi/n_l), hence
        ...
        // Clamped: value flat in n_l -> mixed partial 0.
    ...
    // -> NO change, exact). out.dmu_lR_dnS here is the COMPLETE partial (vdW
    // core 3*mu/nS plus any augmentation term), so the chain is consistent
    // across both contributions. Tangent-only: the converged forward solve uses
...
// repercussion, not a numerical artifact); phi is frozen when forming S1 (a
// keep-phi-live variant remains a possible, not-yet-adopted, alternative).
struct MaxwellConjugateMicroPotentialData
{
    double mu_lR_mech = 0.0;  // J/kg
    double dmu_lR_mech_deps_v =
        0.0;                       // J/kg            per unit volumetric strain
    double dmu_lR_mech_dnl = 0.0;  // J/kg            per unit n_l
    double dmu_lR_mech_drho_lR = 0.0;  // (J/kg)/(kg/m^3)
    bool gate_open = false;
};

// Deprecated: computeIntegrableMechanicalMicroPotential replaces this
// function; nothing currently calls it. The definition is retained
// (historical); do NOT re-wire it.
// Confirmed dead by search: no live callers remain in ProcessLib/ or Tests/
    ...
    double const p_conf, double const Pi, double const rho_lR, double const n_S)
{
    ...
    // SPECIFIC potential [J/kg]; its thermodynamic conjugate is the micro
    // liquid mass per REV volume m_l = rho_lR * phi_m = rho_lR * (1 - phi_M) *
    // n_l, with d m_l / d n_l = rho_lR * n_S. The eigenstress half is already
    // REV-referenced (sigma_sw = -(1 - phi_M)*n_l*Pi), so the conjugate half
    // must divide by the REV micro-liquid mass density rho_lR * n_S, NOT rho_lR
    // alone. Omitting n_S under-references the term by (1 - phi_M): the two
    // cross-partials d sigma_sw,m/d m_l and d mu_lR_mech/d eps_v then differ by
    // (1 - phi_M), the integrability condition d sigma/d m_l = d mu_lR/d eps
    // fails, and the (sigma, mu_lR) response is non-conservative (loop integral
    // != 0).
    if (!(n_S > 0.0))
    {
        OGS_FATAL(
            "computeMaxwellConjugateMicroPotential requires n_S = 1 - phi_M > "
            "0, "
            "got {:g}.",
            n_S);
    }
    MaxwellConjugateMicroPotentialData out;
    // Sharp gate: the term switches on only once the confining pressure
    ...
    // from one Psi (energy-conserving). phi_M is frozen at the GP, so
...
//
// with Pi = -rho_lR*mu_lR_vdw > 0 (disjoining) and p_conf = -tr(sigma_eff)/3 >
// 0 in compression (confining; OGS sigma_eff tension-positive). The vdW helper
...
// Pi_gate = phi_m*Pi = n_S*n_l*Pi (the REV-consistent threshold choice), of
// width w = film_pressure_gate_width. As p_conf rises past
...
//
//   w == 0  -> g = Heaviside(x)  (sharp; reproduces the old gate SHAPE
//   exactly). w  > 0  -> g = smoothstep cubic t^2*(3-2t), t = x/w  (C1: g' = 0
//   at both
...
inline FilmPressureMicroPotentialData computeFilmPressureMicroPotential(
    ...
    double const biot_b)
{
    (void)dmu_lR_vdw_dnl;  // reserved (the film delta's n_l-dependence is via
                           // the gate threshold only; vdW's own dmu_lR_dnl is
                           // folded separately at the call site).
    if (!(rho_lR > 0.0))
    {
        OGS_FATAL(
    ...
    // dPi/dn_l from the vdW chain: Pi = -rho_lR*mu_lR_vdw, density-agnostic
...
//   sigma_sw,m = -phi_m*p_film = -n_S*n_l*(Pi - b*p_conf),   (kept UNCHANGED
// from the film-pressure form above). The integrable partner is
//
//   mu_lR_mech = -[ (Pi + n_l*Pi')*eps_v + 0.5*b*K_drained*eps_v^2 ] / rho_lR
//                                                                    [J/kg]
//   mu_lR = mu_lR_vdw + mu_lR_mech   (ADDITIVE; never overwrites mu_lR_vdw).
//
// with Pi  = -rho_lR*mu_lR_vdw > 0 (disjoining), Pi' = dPi/dn_l, Pi'' =
// d2Pi/dn_l2 (from the vdW + augmentation potential), b = biot_coefficient,
// K_drained the drained bulk modulus (from the elastic stiffness), eps_v the
// volumetric strain.
//
...
//         (p_film = Pi - b*p_conf = Pi + b*K_drained*eps_v on the drained
//         line),
                                          ...
                                          double const rho_lR)
{
    ...
    {
        OGS_FATAL(
            "computeIntegrableMechanicalMicroPotential requires rho_lR > 0, "
            "got "
            "{:g}.",
            rho_lR);
    }
    ...
    // against the linear conjugate A*eps_v/rho_lR = 3.1e-3 J/kg at that deck's
    // Pi ~ -1.6e2 Pa (A = Pi + n_l*Pi' = -2*Pi for the bare cubic core). The
    // quadratic transmitted-load work therefore DOMINATES the linear term at
    // percent-level strain - expected for a stiffness-carrying term, and the
    // reason a sign or scale slip in it would not show up as a small drift.
    out.mu_lR_mech =
        -(A * eps_v + 0.5 * Bmod * eps_v * eps_v) / rho_lR;  // J/kg
    // d(mu_lR_mech)/d(eps_v) = -[ A + B*eps_v ] / rho_lR   [J/kg per unit
    // strain]
    out.dmu_lR_mech_deps_v = -(A + Bmod * eps_v) / rho_lR;
    // d(mu_lR_mech)/d(n_l) = -[ (2*Pi' + n_l*Pi'')*eps_v ] / rho_lR
    //   (dA/dn_l = 2*Pi' + n_l*Pi''; B carries no n_l dependence). [J/kg per
    //   n_l]
    out.dmu_lR_mech_dnl = -((2.0 * dPi_dnl + n_l * d2Pi_dnl2) * eps_v) / rho_lR;
    ...
    double const rho_pi)
{
    ...
    switch (mode)
    {
        ...
        case FilmStrainCouplingMode::Kinematic:
        {
            ...
            double const f = std::max(
                1e-6, 1.0 + kappa * (std::isfinite(eps_v) ? eps_v : 0.0));
            out.w_eff = n_l * f;
            out.dw_eff_dnl = f;
            // kappa frozen at the GP: d w_eff/d eps_v = n_l*kappa on the
            // unclamped branch, 0 when the positivity guard clamps.
            out.dw_eff_deps_v = (f > 1e-6) ? n_l * kappa : 0.0;
            return out;
        }
...
//   S   = 0.5*b*K_drained*eps_v^2      (transmitted-load work; caller may
//                                       drop it via include_S)
//
...
//   mu_mech       = -2*M_v + mu_a*[x*E/kappa - xi0*Gx] -
//   0.5*b*K_d*eps_v^2/rho_lR
//                   with E = exp(-xi0*x) [J/kg]
//   dmu_mech/deps = -2*mu_v*(1+x)^-3 + mu_a*E*(1-xi0-xi0*x) -
//   b*K_d*eps_v/rho_lR dmu_mech/dnl  = 6*mu_v*G3/n_l
    ...
    double const n_l_floor)
{
    ...
    double const xi0 = (potential_augmentation_prefactor > 0.0)
                           ? n_l_eff / (potential_augmentation_exponent * nS *
                                        rho_SR * specific_surface)
    ...
    // Transmitted-load work S (NaN K_drained -> 0, mirrors the shipped
    // partner's sentinel convention).
    double const K_d =
    ...
    // nS*n_l*0.5*b*K_d*eps^2 = 0.5755*0.4245*1.39e3 = 3.4e2 J/m^3 REV.
    ...
    if (!floored)
    {
        ...
        double const Pi_a_weff = (potential_augmentation_prefactor > 0.0)
                                     ? -rho_lR * mu_a * E
                                     : 0.0;  // Pa
        out.dsigma_sw_dnl =
    ...
    out.Psi_film =
        ...
        (-rho_lR *
             (mu_v * G3 +
              ((potential_augmentation_prefactor > 0.0) ? mu_a * Gx : 0.0)) +
