...
def write_native_notebook_project(
    case: Case, project_path: Path, vdw_multiplier: float
) -> dict:
...
def run_native_notebook_case(ogs_bin: Path, case: Case, multiplier: float, idx: int) -> dict:
    case_tag = f"dd{int(case.dry_density)}_native_notebook_{idx:02d}"
    project_path = ROOT / f"{case_tag}.prj"
    meta = write_native_notebook_project(case, project_path, multiplier)
    try:
...
def calibrate_multiplier_for_case(
    ...
    run0 = run_native_notebook_case(ogs_bin, case, 1.0, 0)
    p0 = run0["pressure_mpa"]

    # 2) First scaled guess from near-linear response.
    m1 = target_mpa / max(abs(p0), 1e-12)
    m1 = float(np.clip(m1, 1e-6, 1e18))
    run1 = run_native_notebook_case(ogs_bin, case, m1, 1)
    ...
    while i <= max_iter:
        if abs(p_curr - p_prev) > 1e-12:
            ...
            m_new = m_curr * (target_mpa / max(abs(p_curr), 1e-12))
        m_new = float(np.clip(m_new, 1e-6, 1e18))

        run_new = run_native_notebook_case(ogs_bin, case, m_new, i)
...
def main() -> None:
    ...
    for case in cases:
        ...
        baseline = run_native_notebook_case(args.native_ogs, case, 1.0, 90)
        calibrated = calibrate_multiplier_for_case(
            args.native_ogs, case, target, rel_tol=args.rel_tol
        )
        print(
