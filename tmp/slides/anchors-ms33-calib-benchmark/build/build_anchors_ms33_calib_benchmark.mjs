...
const DECK_ID = "anchors-ms33-calib-benchmark";
const OUT_DIR = "/Users/vinaykumar/git/ogs/Tests/Data/RichardsMechanics/ANCHORS_MS33_presentation_2026-05-20";
const REF_DIR = "/Users/vinaykumar/git/ogs/tmp/slides/anchors-ms33-calib-benchmark/pro-reference-images";
...
const SOURCES = {
  run_summary: "OGS: Tests/Data/RichardsMechanics/ANCHORS_MS33_RUN_SUMMARY_2026-05-20_anchors_dd.md",
  scope_note: "OGS: AGENTS.md (MS33 theoretical benchmarking scope and selected models)",
  eurad_ref: "materialmodels/src/TPM/M33_ANCHORS_v0.pdf (project reference file)",
  postproc_vii: "OGS: ANCHORS_MS33_ModelVII/ms33_postprocess_modelVII.py (total-stress alignment update)",
};

const SLIDES = [
  {
    "kicker": "EURAD-2 / ANCHORS MS33",
    "title": "Calibration And Benchmarking Status",
    "subtitle": "DSM native hierarchical model: Model I calibration plus Model III/IV/VII benchmark replay.",
    "expectedVisual": "Cover with concise status framing and decision-ready summary.",
    "moment": "Model I fit <1%; Models IV/VII still partial",
    "notes": "Open with scope and current readiness: full run completion achieved, but not full benchmark compliance yet.",
    "sources": [
      "run_summary",
      "scope_note",
      "eurad_ref"
  ...
  {
    "kicker": "SCOPE",
    "title": "What Was Simulated",
    "subtitle": "The implemented MS33 suite in this checkout contains six production PRJs across four models.",
    "expectedVisual": "Three cards: implemented models, run setup, and missing case clarity.",
    "cards": [
      [
        "Implemented models",
        "Model I (dd1400, dd1600, dd1800), Model III (gap2mm), Model IV (pellets), and Model VII (freeswelling)."
      ],
      [
        "Replay setup",
        "ANCHORS dry-density calibration basis was used for the replay and scoring, not a fresh Villar refit."
      ],
      [
        "Scope gap",
        "No Model V case exists in the current repository tree; it needs a dedicated ModelV folder and PRJ to be benchmarked."
      ]
    ],
    "notes": "Clarify this prevents confusion about benchmark completeness vs repository completeness.",
    "sources": [
      "scope_note",
      "run_summary",
      "eurad_ref"
  ...
  {
    "kicker": "EXECUTION",
    "title": "Run Completion Snapshot",
    "subtitle": "All implemented cases converged and completed with zero rejected steps.",
    "expectedVisual": "Metric cards for completion, total accepted steps, and output schedule coverage.",
    "metrics": [
      [
        "6 / 6",
        "PRJs completed",
      ...
      [
        "2248",
        "Accepted steps total",
        "Rejected steps: 0"
      ],
      [
        "12/4/12",
        "Output schedules captured",
        "I:12, III:4, IV:4, VII:12 outputs"
      ]
    ],
    "notes": "Use this slide to establish numerical credibility before discussing fit quality.",
    "sources": [
      "run_summary"
    ]
  },
  {
    "kicker": "MODEL I",
    "title": "Dry-Density Calibration Performance",
    "subtitle": "The ANCHORS replay remains tightly aligned with the Villar dry-density target relation.",
    "expectedVisual": "Three cards for dd-specific fit plus aggregate error statement.",
    "cards": [
      [
        "dd1400",
        "Final swelling pressure 1.5147 MPa vs target 1.5038 MPa. Absolute relative error: 0.723%."
      ],
      [
        "dd1600",
        "Final swelling pressure 5.8629 MPa vs target 5.8241 MPa. Absolute relative error: 0.667%."
      ],
      [
        "dd1800",
        "Final swelling pressure 22.3386 MPa vs target 22.5560 MPa. Absolute relative error: 0.964%. Mean error: 0.785%."
      ]
    ],
    "notes": "Directly answer whether calibration is still within a few percent: yes, all three points are within 1%.",
    "sources": [
      "run_summary"
    ]
  },
  {
    "kicker": "MODELS III-IV-VII",
    "title": "Plausibility And Mismatch Review",
    "subtitle": "Model III looks plausible; Model IV and VII still show unresolved macro-state issues.",
    "expectedVisual": "Three diagnostic cards with one key number and one issue statement each.",
    "cards": [
      [
        "Model III (gap2mm)",
        "Final aperture 1.933 mm from 2.000 mm initial (closure 0.0666 mm). Final mean stresses are finite and consistent."
      ],
      [
        "Model IV (pellets)",
        "Block/pellet final mean stress: 8.355 / 2.514 MPa. But transport_porosity goes negative in final state (non-physical)."
      ],
      [
        "Model VII (freeswelling)",
        "Stress-path interpretation is fixed, but simulated void ratio is high (about 2.81 to 2.96) versus benchmark reference band 0.4 to 1.2."
      ]
    ],
    "notes": "Separate convergence success from physics acceptance: numerically stable does not imply benchmark pass.",
    "sources": [
      "run_summary",
      "postproc_vii"
    ]
  },
  {
    ...
    "subtitle": "Current status: strong calibration and run stability, but partial benchmark compliance.",
    "expectedVisual": "Three metric cards summarizing PASS/PARTIAL and principal unresolved points.",
    "metrics": [
      [
        "PASS: 3",
        "Completion, Model I fit, Model III outputs",
        "Run summary score table"
      ],
      [
        "PARTIAL: 2",
        "Model IV and Model VII benchmark outputs",
        "Macro-state and void-ratio gaps remain"
      ],
      [
        "0 FAIL",
        "After Model VII postprocess correction",
        "Remaining issues are behavior mismatch"
      ]
    ],
    "notes": "Highlight that Model VII moved from postprocessing fail to partial after total-stress correction.",
    "sources": [
      "run_summary",
      "postproc_vii"
    ]
  ...
  {
    "kicker": "MODEL VII UPDATE",
    "title": "What Was Fixed In The Postprocessing",
    "subtitle": "Stress comparison now follows benchmark meaning (total stress), and void ratio extraction no longer collapses to zero.",
    "expectedVisual": "Three cards: stress definition fix, porosity extraction fix, and quantitative check.",
    "cards": [
      [
        "Stress definition",
        "Switched from effective sigma_zz to total axial stress: sigma_total = sigma_eff + chi * pL, with chi = S."
      ],
      [
        "Void ratio extraction",
        "Use macro porosity priority: porosity, then dry_density_solid, then transport_porosity fallback."
      ],
      [
        "Quantitative check",
        "All loading steps (200 to 240 d) now match target total stress within approximately +0.003 to +0.011 MPa."
      ]
    ],
    "notes": "This isolates the remaining Model VII issue to constitutive/porosity behavior, not plotting semantics.",
    "sources": [
      "postproc_vii",
      "run_summary"
    ]
  },
  {
    ...
    "subtitle": "Close remaining benchmark gaps in a controlled order before final reporting.",
    "expectedVisual": "Three action cards for scope completion, model corrections, and acceptance closure.",
    "cards": [
      [
        "Add Model V case",
        "Create ANCHORS_MS33_ModelV inputs and postprocess so the repository matches the broader benchmark set."
      ],
      [
        "Investigate macro-state drift",
        "Trace why Model IV gives negative transport_porosity and why Model VII void ratio remains high at loading steps."
      ],
      [
        "Re-run and re-score",
        "Replay all implemented cases with the same ANCHORS-dd baseline and update the scorecard for benchmark submission."
      ]
    ],
    "notes": "Close with a practical sequence that can be executed in short iterations.",
    "sources": [
      "run_summary",
      "scope_note",
      "eurad_ref"
...
function addReferenceCaption(slide, slideNo) {
  addText(
    slide,
    slideNo,
    "Reference pack: EURAD-2 MS33 scope note, ANCHORS run summary, and model-specific postprocessing records.",
    64,
    674,
    980,
    22,
...
async function slideCards(presentation, idx) {
  ...
  for (let cardIdx = 0; cardIdx < cols; cardIdx += 1) {
    ...
    addCard(slide, idx, x, 410, cardW, 204, label, body, { iconKind: iconKinds[cardIdx % iconKinds.length] });
  }
